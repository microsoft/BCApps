...
codeunit 138708 "Record Reference Test Impl." implements "Record Reference"
{
    ...
    var
        ...
        SystemApplicationTestsAppIdTxt: Label '0d60b215-6ee1-4789-8e53-866cfa50c23c', Locked = true;
        RecordReferenceTestsAppIdTxt: Label '8b1d2d7e-cba8-4ac5-a344-00cabbbc6652', Locked = true;
    ...
    var
        RecordReferenceTestImpl: Codeunit "Record Reference Test Impl.";
    begin
        if IsInitialized then
            exit;

        if not (CallerModuleInfo.Id in [SystemApplicationTestsAppIdTxt, RecordReferenceTestsAppIdTxt]) then
            exit;

        if RecordRef.Number in [Database::"Record Reference Test"] then begin
            RecordReference := RecordReferenceTestImpl;
            RecordReferenceTestImpl.SetInitializedCalledModuleId(CallerModuleInfo.Id);
            IsInitialized := true;
        end;
    end;
    ...
    begin
        exit(CallerModuleId in [SystemApplicationTestsAppIdTxt, RecordReferenceTestsAppIdTxt]);
    end;
