...
- **Last Updated**: 2026-06-01 (post-implementation iterations — see Appendix C; latest: quality-pass cleanup — unified the inline API lookup, removed the dead per-type lookups/pages, `TableType = Temporary` buffer — D34, Appendix C #24)
...
**When the platform adds `Enable API Tools` + `Enable Data Query Tools` (or equivalent) to `MCP Configuration`:**

1. `EnableAPITools` / `EnableDataQueryTools` — rewrite each to `GetBySystemId` → set the new field → `Modify` → `LogConfigurationModified`, exactly like the neighbouring `EnableDynamicToolMode`.
2. `IsAPIToolsEnabled` / `IsDataQueryToolsEnabled` — rewrite each to read the new field off `MCP Configuration`.
3. Delete `table 8356 "MCP Feature Activation"` (its ID frees up within the existing `8350–8362` idRange), and remove its permission-set entries — the `table … = X` line in `MCP - Objects` and the `tabledata … = R` / `= IMD` lines in `MCP - Read` / `MCP - Admin`.
4. On `page 8350 "MCP Config List"`, repoint the **API Tools** / **Data Query Tools** status columns from the impl getters (`IsAPIToolsEnabled(Rec.SystemId)` / `IsDataQueryToolsEnabled(...)`) to the real `Rec` fields — they become normal bound columns (and can be made editable). Both are tagged `// MOCK:` in the page.

Nothing else changes. The handlers (`MCP API Tools Feature`, `MCP Data Query Tools Feature`), the Server Features list (including the indented sub-feature and the now-live `APIToolsRequiredForDynamicErr` gate), the Available APIs sub-part visibility on the card, and the `MCP Config` facade all flow through those four procedures. The `<remarks>` MOCK notes on the facade `EnableAPITools` / `EnableDataQueryTools` and the `// MOCK:` note on `MCPConfigCard.APIToolsActive` are comment-only: delete the notes — the code they sit on (the facade delegation and the page-local) stays. (Both are tagged `// MOCK:` so the grep below surfaces them.)
...
**When `MCP Utilities` exposes the real Data Query Tools catalog:** replace the two `InsertTool(...)` calls with a `MCPUtilities.Get…()` lookup, mirroring how `MCP Dyn. Tool Mode Feature.LoadSystemTools` consumes `GetSystemToolsInDynamicMode()`.
