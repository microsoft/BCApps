...
- **Last Updated**: 2026-06-04 (post-implementation iterations — see Appendix C; latest: BC-Platform #44811 merged — productionized the feature (mocks removed, real `MCP Configuration` fields + `GetSystemToolsInDataQuery()` wired in, mock table deleted), fixed the new-config activation default, and added the two new fields to Export/Import — D36–D38; obsoleted the per-type lookup pages rather than deleting them (AS0029 breaking change) — D39)
...
| D36 | **[BC-Platform PR #44811](https://microsoft.ghe.com/bic/BC-Platform/pull/44811) merged — productionized; all mocks removed.** Supersedes the D30 mock table and consumes the D35 pre-staged code. The four `MCP Config Implementation` activation procedures now read/write the real `MCP Configuration.EnableApiTools` / `.EnableAlQueryTools` fields (writes log via `LogConfigurationModified`, guarded by `ConfigurationNotFoundErr`); `MCP Data Query Tools Feature.LoadSystemTools` consumes `"MCP Utilities".GetSystemToolsInDataQuery()`; `MCP Upgrade.EnableApiToolsOnExistingConfigurations` (tag `MS-631012-MCPEnableApiTools-20260603`) turns API Tools on for existing configs on upgrade; `page 8350 "MCP Config List"` status columns bind straight to `Rec.EnableApiTools` / `Rec.EnableAlQueryTools`; the four activation tests assert real field state. Deleted `table 8356 "MCP Feature Activation"` + its three permission-set entries. `grep` for `MOCK:` / `PLATFORM-PENDING` across the MCP app now returns nothing. | #44811 merged. |
| D37 | **New-config activation default fix.** `EnableApiTools` has `InitValue = true`, but a brand-new (unsaved) config read API Tools as *off* because activation reads go through `GetBySystemId`, which misses until the row is inserted — and the Server Features Activate/Deactivate actions errored on the unsaved config. Two parts: (a) `IsAPIToolsEnabled` / `IsDataQueryToolsEnabled` fall back to `MCPConfiguration.Init()` (the table `InitValue`) when the config isn't persisted, so the at-open display is correct; (b) the card's `Name.OnValidate` calls `CurrPage.Update()` when `IsNullGuid(Rec.SystemId)` (not yet inserted), persisting the config the moment it's named so feature actions work without first closing the card. | User testing. |
| D38 | **Export/Import carries the two activation fields.** `ExportConfiguration` writes `enableApiTools` / `enableAlQueryTools`; `ImportConfiguration` reads them back (same `if Contains` guard as the other config booleans; `Active` / `Default` stay deliberately excluded). Round-trip coverage added — `TestExportConfiguration` asserts the JSON keys and `TestImportConfiguration` asserts the imported field values, both with non-default `true` so the assertions bite. | User: keep import/export current with the table. |
| D39 | **Obsoleted the per-type lookup pages instead of deleting them.** D34 deleted `page 8353 "MCP API Config Tool Lookup"` and `page 8367 "MCP Query Config Tool Lookup"`, but they are **public System Application objects** present in the baseline — removing them breaks dependent extensions (`AS0029`). Both were restored verbatim and marked `ObsoleteState = Pending` + `ObsoleteReason` (→ the unified `MCP API Object Lookup`, 8376) + `ObsoleteTag = '29.0'`, wrapped in `#if not CLEAN29 … #endif` (the repo's staged-removal convention — they compile out once `CLEAN29` is defined). The internal `MCP Config Implementation` procedures and the `isTest` library wrappers stay deleted (neither is public surface). | Build error `AS0029`. |
...
25. **Productionization — BC-Platform #44811 merged** (D36–D38). The mock `table 8356 "MCP Feature Activation"` and every `// MOCK:` / `// PLATFORM-PENDING` block are gone. `MCP Config Implementation`'s four activation procedures read/write the real `MCP Configuration.EnableApiTools` / `.EnableAlQueryTools` fields (reads fall back to `Init()` for a not-yet-saved config — D37); `MCP Data Query Tools Feature.LoadSystemTools` consumes `"MCP Utilities".GetSystemToolsInDataQuery()`; `MCP Upgrade.EnableApiToolsOnExistingConfigurations` flips existing configs on (tag `MS-631012-...`); `page 8350`'s status columns bind to `Rec.EnableApiTools` / `Rec.EnableAlQueryTools`; the four activation tests assert real state; the deleted table's permission-set entries were removed. New-config fix (D37): the card's `Name.OnValidate` persists the config via `CurrPage.Update()` once it's first named (`IsNullGuid(Rec.SystemId)`). Export/Import (D38) now carries both fields, with round-trip test coverage.
26. **Obsoleted the per-type lookup pages** (D39). `page 8353 "MCP API Config Tool Lookup"` / `page 8367 "MCP Query Config Tool Lookup"` — deleted in D34 — are public baseline objects, so deleting them tripped `AS0029`. Restored both verbatim with `ObsoleteState = Pending` + `ObsoleteReason` (→ `MCP API Object Lookup`) + `ObsoleteTag = '29.0'`, wrapped in `#if not CLEAN29`. See Appendix D #14.
...
14. **Removing a public object from the System Application is a breaking change (`AS0029`) — obsolete it, don't delete it.** Public objects that shipped in a previous version must stay in the symbol surface for dependent extensions. To retire one, keep the object and set `ObsoleteState = Pending` + `ObsoleteReason` + `ObsoleteTag = '<version>.0'`, wrapped in `#if not CLEAN<version> … #endif` (the repo's staged-removal convention; it compiles out in a `CLEAN<version>` build whose baseline also lacks it). Only **internal**-access members and `isTest`-project code can be deleted outright. We deleted public pages 8353/8367 (D34) and had to restore + obsolete them (D39).

## Appendix E — Productionization: mocks removed (completed)

> **✅ Done — [BC-Platform PR #44811](https://microsoft.ghe.com/bic/BC-Platform/pull/44811) merged, and this PR productionized against it.** Every `// MOCK:` / `// PLATFORM-PENDING` placeholder was removed and wired to the real platform members; `grep -rn "MOCK:\|PLATFORM-PENDING" "src/System Application/App/MCP/"` now returns nothing. Kept as a record of what the two placeholders were and how they were retired (decision log D36).

### 1. API Tools / Data Query Tools activation persistence — done

**What was mocked & why:** the two activation flags belong on the platform-owned `MCP Configuration` table, which the app can't extend, and no lower-layer table is reachable from the System App (Appendix D #12). During development they were persisted in app-owned `table 8356 "MCP Feature Activation"`, reached only through four procedures in `codeunit 8351 "MCP Config Implementation"`.

**#44811 added** `field(8; EnableApiTools)` (`InitValue = true`) and `field(9; EnableAlQueryTools)` to `MCP Configuration` (the platform keeps the "AL Query Tools" name; our "Data Query Tools" UX maps onto `EnableAlQueryTools`). Retired by:

1. The four procedures (`EnableAPITools` / `EnableDataQueryTools` / `IsAPIToolsEnabled` / `IsDataQueryToolsEnabled`) now read/write the real fields directly — writes through `LogConfigurationModified`; reads fall back to `Init()` for a not-yet-saved config so a new config reflects the `InitValue` default (D37).
2. `table 8356 "MCP Feature Activation"` deleted, with its permission-set entries (`MCP - Objects` `= X`; `MCP - Read` `= R`; `MCP - Admin` `= IMD`).
3. `codeunit 8356 "MCP Upgrade".EnableApiToolsOnExistingConfigurations` sets `EnableApiTools := true` on every existing configuration on upgrade (`InitValue` only covers new ones; tag `MS-631012-MCPEnableApiTools-20260603`).
4. `page 8350 "MCP Config List"`'s API Tools / Data Query Tools status columns bind straight to `Rec.EnableApiTools` / `Rec.EnableAlQueryTools`.
5. `codeunit 130130 "MCP Config Test"`'s four activation tests assert the real field state.

Everything else — the handlers, the Server Features list (incl. the indented sub-feature and the live `APIToolsRequiredForDynamicErr` gate), the Available APIs visibility on the card, and the `MCP Config` facade — flowed through those four procedures unchanged.

### 2. Data Query Tools system tool catalog — done

**What was mocked:** `MCP Utilities` didn't expose the Data Query Tools system tools, so `codeunit 8368 "MCP Data Query Tools Feature".LoadSystemTools` hardcoded a two-row preview (`compile_al_query`, `run_al_query`).

**#44811 added** `"MCP Utilities".GetSystemToolsInDataQuery(): Dictionary of [Text, Text]`. `LoadSystemTools` now consumes it (mirroring `MCP Dyn. Tool Mode Feature.LoadSystemTools` + `GetSystemToolsInDynamicMode()`).

> The Data Query Tools **runtime** (compiling and executing the client-submitted AL queries on the server) remains a separate, larger platform dependency — this feature only *configures and advertises* the tools.
