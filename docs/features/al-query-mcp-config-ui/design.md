...
- **Last Updated**: 2026-06-01 (post-implementation iterations — see Appendix C; latest: tried + reverted a `Tools` FastTab for the API list — BC columnizes grouped parts — D32, Appendix C #22, Appendix D #13)
- **Naming note (2026-05)**: The feature originally called "AL Query Server" was renamed to "AL Query Tools" (D21). Historical decision-log entries below predate the rename and use the old name; current code, captions, and procedures all use "AL Query Tools".
...
### 1. API Tools / AL Query Tools activation persistence

**Why it's mocked:** the two activation flags belong on the platform-owned `MCP Configuration` table, which the app can't extend, and no lower-layer table is reachable from the System App (Appendix D #12). They're persisted instead in app-owned `table 8360 "MCP Feature Activation"` (`Config Id` + two booleans), reached **only** through four procedures in `codeunit 8351 "MCP Config Implementation"`.

**When the platform adds `Enable API Tools` + `Enable AL Query Tools` (or equivalent) to `MCP Configuration`:**

1. `EnableAPITools` / `EnableALQueryTools` — rewrite each to `GetBySystemId` → set the new field → `Modify` → `LogConfigurationModified`, exactly like the neighbouring `EnableDynamicToolMode`.
...
**Why it's mocked:** `MCP Utilities` doesn't yet expose the AL Query system tools, so `codeunit 8368 "MCP AL Query Tools Feature".LoadSystemTools` hardcodes a two-row preview (`compile_al_query`, `run_al_query`).

**When `MCP Utilities` exposes the real AL Query catalog:** replace the two `InsertTool(...)` calls with a `MCPUtilities.Get…()` lookup, mirroring how `MCP Dyn. Tool Mode Feature.LoadSystemTools` consumes `GetSystemToolsInDynamicMode()`.

> The AL Query **runtime** (compiling and executing client-submitted AL on the server) is a separate, larger platform dependency. It isn't represented by a code mock here — this feature only *configures and advertises* the tools.
