<#
.Synopsis
    Builds and writes the per-branch unstable tests artifact from a given set of CI/CD runs.
...
.Parameter RunIds
    The CI/CD (or PR Build) run ids to pull failing tests from.
...
if ($RunIds.Count -eq 0) {
    ...
    Write-Host "::warning::No CI/CD runs provided for branch '$Branch'. Keeping existing unstable tests list unchanged."
    return
}
