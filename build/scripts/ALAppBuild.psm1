...
class BuildMetadataProvider
{
    ...
    static hidden LoadBuildInfo()
    {
        ...
        {
            [BuildMetadataProvider]::ApplicationBuildInfo = Get-Content -Path "$ENV:INETROOT\Eng\Core\Build\projects.json" -Raw | ConvertFrom-Json
            [BuildMetadataProvider]::ApplicationGroupInfo = Get-Content -Path "$ENV:INETROOT\Eng\Core\Build\groups.json" -Raw | ConvertFrom-Json
        }
    }
    ...
    hidden static [PSCustomObject[]] GetApplicationsInGroup([string]$GroupName, [string] $CountryCode)
    {
        ...
        {
            throw "The group '$GroupName' could not be found. Have you updated Eng\Core\Build\groups.json ?"
        }
    ...
    static [BuildMetadata] GetProject([string]$CountryCode, [string]$ApplicationName, [switch] $StrictMode)
    {
        ...
        $buildMetadata.ProjectFolder = $global:ExecutionContext.InvokeCommand.ExpandString($projectInfo.projectPath)
        $buildMetadata.AppJsonPath = Join-Path $buildMetadata.ProjectFolder "app.json"
        if($projectInfo.appJsonPath) {
            $buildMetadata.AppJsonPath = $global:ExecutionContext.InvokeCommand.ExpandString($projectInfo.appJsonPath)
        }
        if(-not (Test-Path $buildMetadata.AppJsonPath -PathType Leaf))
        {
            throw "The app.json file $($buildMetadata.AppJsonPath) does not exist."
        }
        $buildMetadata.PackageCacheFolder = $global:ExecutionContext.InvokeCommand.ExpandString($projectInfo.packageCacheFolder)
        $buildMetadata.PackageOutputFolder = $global:ExecutionContext.InvokeCommand.ExpandString($projectInfo.packageOutputFolder)
        $buildMetadata.BuildArtifactsOutputFolder = $global:ExecutionContext.InvokeCommand.ExpandString($projectInfo.buildArtifactsOutputFolder)
        ...
        if ($StrictMode)
        {
            $buildMetadata.RuleSetPath = $global:ExecutionContext.InvokeCommand.ExpandString($projectInfo.ruleSetPathMinorRelease)
        }
        else
        {
            $buildMetadata.RuleSetPath = $global:ExecutionContext.InvokeCommand.ExpandString($projectInfo.ruleSetPath)
        ...
        $buildMetadata.AssemblyProbingPaths = @($projectInfo.assemblyProbingFolders | ForEach-Object {
            $global:ExecutionContext.InvokeCommand.ExpandString($_)
        })
