...
codeunit 154 "Navigation Bar Subscribers"
{
    ...
    /// <summary>Notifies that the Default Open Copilot Capabilities has been invoked.</summary>
    /// <param name="Handled">The flag which if set, would stop executing the event.</param>
    [IntegrationEvent(false, false)]
    internal procedure OnBeforeDefaultOpenCopilotCapabilities(var Handled: Boolean)
    begin
    end;
}