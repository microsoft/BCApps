...
report 6952 "Expense Report Summary Page"
{
    ...
    dataset
    {
        dataitem("Expense Report Header"; "Expense Report Header")
        {
            ...
            dataitem(MileageLine; "Expense Report Line")
            {
                ...
                trigger OnAfterGetRecord()
                begin
                    AmountPerMileage := GetMileageRate("Expense Date", "Expense Currency Code", "Expense Currency Factor");
    ...
    local procedure GetMileageRate(ExpenseDate: Date; CurrencyCode: Code[10]; CurrencyFactor: Decimal): Decimal
    var
        ExpenseAgentSetup: Record "Expense Agent Setup";
    begin
        if ExpenseAgentSetup.Get() then
            exit(ExpenseAutoPopulation.GetStandardRateOfMileage(ExpenseDate, CurrencyCode, CurrencyFactor, ExpenseAgentSetup."Standard Rate of Mileage"));
    end;
