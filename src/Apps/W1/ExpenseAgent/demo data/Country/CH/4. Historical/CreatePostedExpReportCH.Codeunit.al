...
codeunit 8443 "Create Posted Exp. Report CH"
{
    SingleInstance = true;
    ...
    begin
        AccountNo := ExpenseGLAccount.FindGLAccountByName(ExpenseGLAccountNamesCH.JobSalesAppliedAccountName());
    end;
    ...
    var
        ...
        CreateExpCategoriesCH: Codeunit "Create Exp. Categories CH";
    ...
    begin
        ...
        Expense := ContosoExpenseAgent.InsertExpense(CreateExpenseUser.JO(), CreateExpCategoriesCH.PerDiem(), CreateExpenseLocation.GermanyAll(), BusinessTripToHamburgLbl, '', ContosoUtility.AdjustDate(19021103D), '', 0, '', CreateExpensePaymentMethod.Cash(), true, false, '', CreateDateTime(ContosoUtility.AdjustDate(19021024D), 064500T), CreateDateTime(ContosoUtility.AdjustDate(19021027D), 161500T), 0, 0, '', '', '', '', '');
        ...
        ReleaseAndAddExpenseToExpenseReport(Expense, ExpenseReportHeader);
        Expense := ContosoExpenseAgent.InsertExpense(CreateExpenseUser.EH(), CreateExpCategoriesCH.PerDiem(), CreateExpenseLocation.UKOther(), TripToUKLbl, '', ContosoUtility.AdjustDate(19021121D), '', 0, '', CreateExpensePaymentMethod.Cash(), true, false, '', CreateDateTime(ContosoUtility.AdjustDate(19021114D), 053000T), CreateDateTime(ContosoUtility.AdjustDate(19021118D), 220500T), 0, 0, '', '', '', '', '');
        UpdateExpensePerDiem(Expense."No.", 20000, false, false, true);
        UpdateExpensePerDiem(Expense."No.", 40000, false, true, false);
        ReleaseAndAddExpenseToExpenseReport(Expense, ExpenseReportHeader);
        Expense := ContosoExpenseAgent.InsertExpense(CreateExpenseUser.EH(), CreateExpCategoriesCH.PerDiem(), CreateExpenseLocation.Domestic(), TripToDoverLbl, '', ContosoUtility.AdjustDate(19021204D), '', 0, '', CreateExpensePaymentMethod.Cash(), true, false, '', CreateDateTime(ContosoUtility.AdjustDate(19021127D), 070000T), CreateDateTime(ContosoUtility.AdjustDate(19021129D), 145500T), 0, 0, '', '', '', '', '');
        ReleaseAndAddExpenseToExpenseReport(Expense, ExpenseReportHeader);
