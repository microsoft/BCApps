...
codeunit 8244 "Create Posted Exp. Report GB"
{
    SingleInstance = true;
    ...
    begin
        AccountNo := ExpenseGLAccount.FindGLAccountByName(ExpenseGLAccountNamesGB.SaleofResourcesName());
    end;
    ...
    begin
        ...
        Expense := ContosoExpenseAgent.InsertExpense(CreateExpenseUser.JO(), CreateExpCategoriesGB.PerDiem(), CreateExpenseLocation.GermanyAll(), BusinessTripToHamburgLbl, '', ContosoUtility.AdjustDate(19021103D), '', 0, '', CreateExpensePaymentMethod.Cash(), true, false, '', CreateDateTime(ContosoUtility.AdjustDate(19021024D), 064500T), CreateDateTime(ContosoUtility.AdjustDate(19021027D), 161500T), 0, 0, '', '', '', '', '');
