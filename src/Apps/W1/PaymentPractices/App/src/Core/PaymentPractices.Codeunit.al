...
codeunit 689 "Payment Practices"
{
    var
        ...
        BlankPaymentPeriodCodeErr: Label 'You must select a Payment Period Code before generating.';
        NoTemplatesForSchemeErr: Label 'No payment period templates exist for the selected reporting scheme. Create a template first.';
    ...
    begin
        ...
        if PaymentPracticeHeader."Payment Period Code" = '' then
            GuardBlankPaymentPeriodCode(PaymentPracticeHeader."Reporting Scheme");
    ...
    local procedure GuardBlankPaymentPeriodCode(ReportingScheme: Enum "Paym. Prac. Reporting Scheme")
    var
        PaymentPeriodHeader: Record "Payment Period Header";
        NoTemplatesErr: ErrorInfo;
    begin
        PaymentPeriodHeader.SetRange("Reporting Scheme", ReportingScheme);
        if not PaymentPeriodHeader.IsEmpty() then
            Error(BlankPaymentPeriodCodeErr);

        NoTemplatesErr.Message := NoTemplatesForSchemeErr;
        NoTemplatesErr.PageNo := Page::"Payment Period List";
        NoTemplatesErr.AddNavigationAction(NoTemplatesForSchemeErr);
        Error(NoTemplatesErr);
    end;
}
