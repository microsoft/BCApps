codeunit 139672 "Hybrid BC Last Management Test"
{
    ...
    begin
        ...
        SyncedVersion := 100;
        LibraryHybridBCLast.InsertNotification(RunId, TriggerType, '', SyncedVersion);
    ...
    begin
        ...
        // [THEN] The 15.0, 16.0, 17.0, 18.0 , 19.0, 20.0 and 21.0 target upgrade version is returned
        ...
        Assert.IsTrue(TargetVersions.Contains(21.0), '21.0 not returned as target version');
        Assert.AreEqual(7, TargetVersions.Count(), 'Incorrect number of target versions returned.');

        // [GIVEN] Migration is set up with BC 14.9 as source
        TargetVersions.RemoveRange(1, 7);
        Initialize(14.0);
        ...
        // [THEN] The 16.0, 17.0, 18.0 , 19.0, 20.0 and 21.0 target upgrade version is returned
        Assert.IsTrue(TargetVersions.Contains(16.0), '16.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(17.0), '17.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(18.0), '18.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(19.0), '19.0 not returned as target version');
        ...
        // [THEN] The 17.0, 18.0 , 19.0, 20.0 and 21.0 target upgrade version is returned
        Assert.IsTrue(TargetVersions.Contains(17.0), '17.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(18.0), '18.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(19.0), '19.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(20.0), '20.0 not returned as target version');
        ...
        // [THEN] The 18.0 , 19.0, 20.0 and 21.0 target upgrade version is returned
        Assert.IsTrue(TargetVersions.Contains(18.0), '18.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(19.0), '19.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(20.0), '20.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(21.0), '21.0 not returned as target version');
        ...
        // [THEN] Only 19.0, 20.0 and 21.0 target upgrade version is returned
        Assert.IsTrue(TargetVersions.Contains(19.0), '19.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(20.0), '20.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(21.0), '21.0 not returned as target version');
        ...
        // [THEN] Only 20.0 and 21.0 target upgrade version is returned
        Assert.IsTrue(TargetVersions.Contains(20.0), '20.0 not returned as target version');
        Assert.IsTrue(TargetVersions.Contains(21.0), '21.0 not returned as target version');
        Assert.AreEqual(2, TargetVersions.Count(), 'Incorrect number of target versions returned.');
        TargetVersions.RemoveRange(1, 2);

        // [GIVEN] Migration is set up with BC 20.0 as source
        Initialize(20.0);

        // [WHEN] A call to get upgrade versions is called
        W1Management.GetSupportedUpgradeVersions(TargetVersions);

        // [THEN] Only 21.0 target upgrade version is returned
        Assert.IsTrue(TargetVersions.Contains(21.0), '21.0 not returned as target version');
    ...
    begin
        ...
        LibraryHybridBCLast.VerifyEventOrder(20.0, false); // 20x upgrade
        LibraryHybridBCLast.VerifyEventOrder(21.0, true); // 20x upgrade
    end;
    ...
    begin
        ...
        LibraryHybridBCLast.VerifyEventOrder(19.0, false); // 19x upgrade
        LibraryHybridBCLast.VerifyEventOrder(20.0, false); // 20x upgrade
        LibraryHybridBCLast.VerifyEventOrder(21.0, true); // 21x upgrade
    end;
