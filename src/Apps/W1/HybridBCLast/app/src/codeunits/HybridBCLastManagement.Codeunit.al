...
codeunit 4021 "Hybrid BC Last Management"
{
    var
        ...
        FailurePreparingDataErr: Label 'Failed to prepare data for the table. Inner error: %1', Comment = '%1 = Inner error';
        FailureCopyingTableErr: Label 'Failed to copy the table. Inner error: %1', Comment = '%1 = Inner error';
...
#pragma warning restore AA0245
    ...
    begin
        ...
        if not (IntelligentCloudSetup."Product ID" = HybridBCLastWizard.ProductId()) then
            exit(false);
