...
5. Each reasoning must follow the **fixed professional template**.
...
* `reasoning`:
  A concise, audit-ready justification following the **fixed reasoning template**:
  ...
  * Keep to **one or two sentences**, written in clear, professional English.
  ...
  * If the vendor name is generic or uninformative, **omit it**.
...
* Apply **vendor knowledge** when it clearly signals expense type.
  ...
  * Example: *Microsoft* → IT/software; *Lufthansa* → travel; *Deloitte* → professional services.
* Ignore vendor names that are generic or uninformative (e.g., “ABC Holding,” “Nordic Trade ApS”).
* Apply **general accounting logic** (capital vs. expense, goods vs. services).
* Do not rely on internal company policies or historical data.
