...
*Updated: 2026-07-29 -- Added idempotent export-step behavior.*
...
*Updated: 2026-07-29 -- Clarified that both Export and Send avoid duplicate exports.*
...
- The `DoSend` path and the explicit Export response both check `E-Document Service Status`. If the service is already `Exported`, Send goes straight to the integration send and Export advances the workflow without re-exporting.
