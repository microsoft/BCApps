...
page 6183 "E-Doc. Purchase Draft Subform"
{
    ...
    layout
    {
        area(Content)
        {
            ...
            {
                field(OrderLineMatch; OrderLineMatch)
                {
                    ApplicationArea = All;
                    Caption = 'Order matched';
                    ToolTip = 'Specifies the order line that matches this received invoice line.';
                    Editable = false;
                    Visible = HasLinesMatched;
                    trigger OnDrillDown()
                    var
                        PurchaseHeader: Record "Purchase Header";
                    begin
                        if not PurchaseHeader.Get("Purchase Document Type"::Order, MatchedOrderLine."Document No.") then
                            exit;
            ...
            group("&Line")
            {
                ...
                group("Related Information")
                {
                    ...
                    action(MatchToOrderLine)
                    {
                        ...
                        Image = Line;
                        ToolTip = 'Specify the order line to which this invoice line should be matched.';
                        Scope = Repeater;
                        ...
                        var
                            ...
                            AvOrderLines: Page "Candidate Order Lines for Inv.";
                        begin
                            EDocumentPurchaseHeader.Get(Rec."E-Document Entry No.");
                            AvOrderLines.SetEDocumentPurchaseLine(Rec);
                            AvOrderLines.LookupMode := true;
                            if AvOrderLines.RunModal() <> Action::LookupOK then
                                exit;
                            AvOrderLines.GetRecord(PurchaseLine);
                            Rec.Validate("[BC] Order Line SystemId", PurchaseLine.SystemId);
                            Rec.Modify();
                            CurrPage.Update();
                        end;
                    }
                    action(RemoveMatchedLine)
                    {
                        ApplicationArea = All;
                        Caption = 'Remove match to order line';
                        Image = Delete;
                        ToolTip = 'Remove the link to the matched order line.';
                        Scope = Repeater;
                        Enabled = CurrentRecHasOrderLineMatch;

                        trigger OnAction()
                        begin
                            Clear(Rec."[BC] Order Line SystemId");
                            Rec.Modify();
                            CurrPage.Update();
                        end;
                    }
    ...
    var
        ...
        AdditionalColumns, OrderLineMatch : Text;
        LineAmount: Decimal;
        DimVisible1, DimVisible2, CurrentRecHasOrderLineMatch, HasAdditionalColumns, HasLinesMatched : Boolean;
    ...
    var
        OrderMatchLbl: Label 'Order %1 - %2', Comment = '%1 = Order No., %2 = Order Line Description';
    begin
        Clear(MatchedOrderLine);
        ...
        CurrentRecHasOrderLineMatch := MatchedOrderLine.GetBySystemId(Rec."[BC] Order Line SystemId");
        if CurrentRecHasOrderLineMatch then
            OrderLineMatch := StrSubstNo(OrderMatchLbl, MatchedOrderLine."Document No.", MatchedOrderLine.Description)
        else
            OrderLineMatch := 'No order line matched';
        HasLinesMatched := HasLinesMatched or CurrentRecHasOrderLineMatch;
    end;
