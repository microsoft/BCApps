...
1. **Validation**: Checks that all draft lines have a type and number, and that matched lines have UOM info. Receivability is no longer a blocking check -- whether matched order lines are received yet is surfaced as an informational warning only.
2. **Invoice creation**: Creates a `Purchase Header` (type Invoice), sets vendor, dates, currency, and vendor invoice number. Checks for duplicate external document numbers. Creates one purchase invoice line per draft line, in line order.
3. **PO match transfer**: `TransferPOMatchesFromEDocumentToInvoice()` moves match records from the E-Document to the purchase invoice as BaseApp `"Matched Order Line"` records. It distributes the e-document line quantity (in base UoM) across the matched order lines: each explicitly matched receipt is invoiced for its received-not-invoiced quantity, then any remaining budget soaks up other received-not-invoiced receipts on the order line (preferring existing receipts before receiving on invoice), and whatever is still left is received on invoice -- capped at the order line's outstanding quantity, so existing receipts and a receive-on-invoice remainder can coexist on one order line. The receive-on-invoice remainder is recorded explicitly in the match's `"Qty. to Receive"`, which posting uses to drive the implicit receipt.
