...
page 6181 "E-Document Purchase Draft"
{
    ...
    actions
    {
        area(Processing)
        {
            ...
            action(ViewOrderLinesForSelectedVendor)
            {
                ApplicationArea = Basic, Suite;
                Caption = 'View order lines for the vendor';
                ToolTip = 'See unmatched order lines for the selected vendor.';
                Image = ViewDetails;

                trigger OnAction()
                var
                    Vendor: Record Vendor;
                    VendorOrderLinesPage: Page "Vendor Order Lines";
                begin
                    if not Vendor.Get(EDocumentPurchaseHeader."[BC] Vendor No.") then
                        exit;

                    VendorOrderLinesPage.SetVendor(Vendor."No.");
                    VendorOrderLinesPage.Run();
                end;
            }
        }
        area(Navigation)
