...
4. During Finish Draft, `TransferPOMatchesFromEDocumentToInvoice()` transfers the matches onto the created purchase invoice as BaseApp `"Matched Order Line"` (table 5817) records via `"Matched Order Line Mgmt.".CreateMatchedOrderLine()`.

**Transfer / distribution algorithm:** `TransferPOMatchesFromEDocumentToInvoice()` treats the e-document line quantity (in base UoM) as a budget to distribute across the matched order lines. For each matched order line it first transfers the explicitly matched posted receipts -- each invoices its `"Qty. Rcd. Not Invoiced"`, which is subtracted from the budget -- then, while budget remains, it soaks up the order line's other received-not-invoiced receipts (in natural order, partial-capping the last one) so existing receipts are preferred over a new receive on invoice, and finally receives whatever is still left on invoice, **capped at the order line's `"Outstanding Qty. (Base)"`** (so it never over-receives) and converted back to the order line's UoM with `"Unit of Measure Management".CalcQtyFromBase()` (a missing conversion factor falls back to 1:1). Receipts and a receive-on-invoice remainder therefore **coexist** on a single order line: the receipt rows carry `Receipt on Invoice = false`, the remainder row carries the order line's `"Receipt on Invoice"` flag and a null receipt SystemId. The remainder row also records its receive quantity explicitly in the match's `"Qty. to Receive"` (receipt rows record zero), so posting drives the implicit receipt directly from that field instead of reconstructing it from the quantity to invoice.

**Key data:** Matches are staged in `"E-Doc. Purchase Line PO Match"` (table 6114) -- a junction table linking e-document lines to PO lines and receipt lines via SystemIds -- and transferred to BaseApp's `"Matched Order Line"` table when the invoice is created. Warnings go in `"E-Doc. PO Match Warning"` (table 6115). Receipt-on-invoice behavior is driven by the BaseApp vendor field `"Receipt on Invoice Policy"` (Manual / Enabled), which is inherited onto the purchase order header and lines and can be overridden per order line; it is enforced per **line** (a line can be receipt-on-invoice while sibling lines are not, and a partially received line can still be flagged). Matching a not-yet-received line is allowed and only surfaced as a warning.
