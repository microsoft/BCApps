...
codeunit 139547 "E-Doc. Remit. Advice Test"
{
    ...
    [Test]
    procedure PostedPaymentWithSameDocumentRefundIsExportedCorrectly()
    var
        PaymentVendLedgEntry: Record "Vendor Ledger Entry";
    ...
    begin
        // [SCENARIO] A posted vendor payment with a same-document refund is exported
        // as an electronic remittance advice with the refund amount and document reference.
        Initialize();
        EnsureCompanyPeppolData();
        EnsureVendorPeppolData();
        SetServiceDocumentFormat(Enum::"E-Document Format"::"PEPPOL BIS 3.0");
        ...
        // [GIVEN] A refund for the same vendor sharing the payment's document number.
        CreateRefundWithPaymentDocumentNo(RefundVendLedgEntry, Vendor."No.", PaymentVendLedgEntry."Document No.");

        // [GIVEN] The refund has an applied refund detailed entry.
        RefundAmount := CreateDetailedVendorLedgerEntry(RefundVendLedgEntry, RefundVendLedgEntry."Entry No.", DetailedVendorLedgEntry."Entry Type"::Application, DetailedVendorLedgEntry."Document Type"::Refund, 1);
        PaymentVendLedgEntry.CalcFields(Amount);
        ExpectedTotalPaid := Abs(PaymentVendLedgEntry.Amount) - Abs(RefundAmount);

        // [WHEN] The posted payment is exported.
        Assert.IsTrue(EDocRemitAdviceExport.ExportFromPostedPayment(PaymentVendLedgEntry, false), 'The posted payment should be exported successfully.');

        // [THEN] The exported e-document contains the expected remittance advice XML.
        Assert.IsTrue(FindEDocumentForRecord(EDocument, PaymentVendLedgEntry), 'An e-document should be created for the posted payment.');

        EDocumentLog.GetDocumentBlobFromLog(EDocument, EDocumentService, TempBlob, Enum::"E-Document Service Status"::Exported);
        Assert.IsTrue(TempBlob.HasValue(), 'The exported remittance advice blob should have content.');

        AssertRemittanceAdviceRefundXml(TempBlob, PaymentVendLedgEntry."Document No.", ExpectedTotalPaid, RefundAmount, RefundVendLedgEntry."Document No.");
        SetServiceDocumentFormat(Enum::"E-Document Format"::Mock);
    end;

    local procedure AssertRemittanceAdviceRefundXml(var TempBlob: Codeunit "Temp Blob"; ExpectedPaymentDocNo: Code[20]; ExpectedTotalPaid: Decimal; ExpectedRefundAmount: Decimal; ExpectedRefundDocNo: Code[20])
    var
    ...
    begin
        ...
        // [THEN] The exported document is a RemittanceAdvice document.
        ...
        // [THEN] The payment document number is exported.
        ...
        Assert.AreEqual(ExpectedPaymentDocNo, XmlNode.AsXmlElement().InnerText(), 'The RemittanceAdvice ID should be the payment document no.');

        // [THEN] TotalPaymentAmount reflects the payment less the refund.
        ExpectedAmountText := Format(Round(ExpectedTotalPaid, 0.01), 0, 9);

        Assert.IsTrue(XmlDoc.SelectSingleNode('/ra:RemittanceAdvice/cbc:TotalPaymentAmount', XmlNsManager, XmlNode), 'The document should have a TotalPaymentAmount element.');
        Assert.AreEqual(ExpectedAmountText, XmlNode.AsXmlElement().InnerText(), 'TotalPaymentAmount should be reduced by the refund amount.');

        // [THEN] The refund is exported as a CreditLineAmount.
        ...
        // [THEN] The refund document number is exported as the credit document reference.
        ...
        Assert.AreEqual(ExpectedRefundDocNo, XmlNode.AsXmlElement().InnerText(), 'The credit document reference should contain the refund document no.');
    end;
    ...
    local procedure CreateDetailedVendorLedgerEntry(VendorLedgerEntry: Record "Vendor Ledger Entry"; AppliedVendorLedgerEntryNo: Integer; EntryType: Enum "Detailed CV Ledger Entry Type"; DocumentType: Enum "Gen. Journal Document Type"; Sign: Integer): Decimal
    var
        DetailedVendorLedgEntry: Record "Detailed Vendor Ledg. Entry";
    ...
    begin
        if ExistingDetailedVendorLedgEntry.FindLast() then
            DetailedVendorLedgEntry."Entry No." :=
                ExistingDetailedVendorLedgEntry."Entry No." + 1
        else
