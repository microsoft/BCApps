...
2. **Review page** — the single canonical review surface for an order. It summarizes what Copilot did (resolved Tax Area, ship-to context, and per-line Tax Jurisdiction Codes with AI confidence indicators) and hosts the **Review and Approve** (blocking) / **Review** (non-blocking) action, marked with the Copilot icon.

3. **Persistent badge on the BC Sales Order** — a `Copilot Tax Match Applied` Boolean propagates from the Shopify order onto the Sales Header that BC creates from it. The Sales Order page shows it as a read-only field plus a "Show Copilot Tax Decisions" action that opens the review page.
