...
2. **Review page** — the single canonical review surface for an order. It summarizes what Copilot did (resolved Tax Area, ship-to context, each tax line with the item it taxes and its Tax Jurisdiction Code + AI confidence indicators) and hosts a single **Approve** action.
