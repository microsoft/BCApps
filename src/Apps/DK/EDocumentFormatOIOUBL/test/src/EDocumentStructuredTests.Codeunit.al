...
codeunit 13851 "E-Document Structured Tests"
{
    ...
    var
        ...
        OIOUBLStructuredValidations: Codeunit "OIOUBL Structured Validations";
    ...
    #region OIOUBL XML
    [Test]
    procedure TestOIOUBLInvoice_ValidDocument()
    var
        EDocument: Record "E-Document";
    begin
        // [FEATURE] [E-Document] [OIOUBL] [Content Extraction]
        // [SCENARIO] Import and extract content from a valid OIOUBL invoice document

        // [GIVEN] A valid OIOUBL XML invoice document is imported
        ...
        if ProcessEDocumentToStep(EDocument, "Import E-Document Steps"::"Read into Draft") then begin
            OIOUBLStructuredValidations.SetMockCurrencyCode(MockCurrencyCode);
            OIOUBLStructuredValidations.SetMockDate(MockDate);

            // [THEN] All document content is correctly extracted and validated
    ...
    procedure TestOIOUBLInvoice_ValidDocument_ViewExtractedData()
    var
        EDocument: Record "E-Document";
        EDocImport: Codeunit "E-Doc. Import";
    begin
        // [FEATURE] [E-Document] [OIOUBL] [View Data]
        // [SCENARIO] View extracted data from a valid OIOUBL invoice document
    ...
    [Test]
    procedure TestOIOUBLInvoice_ValidDocument_PurchaseInvoiceCreated()
    var
        EDocument: Record "E-Document";
    ...
    begin
        // [FEATURE] [E-Document] [OIOUBL] [Purchase Invoice Creation]
        // [SCENARIO] Create a purchase invoice from a valid OIOUBL invoice document
        ...
        // [THEN] The purchase header is correctly created with OIOUBL data
        OIOUBLStructuredValidations.SetMockCurrencyCode(MockCurrencyCode);
        OIOUBLStructuredValidations.SetMockDate(MockDate);
    ...
    [Test]
    procedure TestOIOUBLInvoice_ValidDocument_UpdateDraftAndFinalize()
    var
        EDocument: Record "E-Document";
        PurchaseHeader: Record "Purchase Header";
        Item: Record Item;
        ...
        VariantRecord: Variant;
    begin
        // [FEATURE] [E-Document] [OIOUBL] [Draft Update]
        // [SCENARIO] Update draft purchase document data and finalize processing

        // [GIVEN] A valid OIOUBL XML invoice document is imported and processed to draft preparation
        Initialize(Enum::"Service Integration"::"No Integration");
        Vendor."VAT Registration No." := 'GB123456789';
        Vendor.Modify(true);
        SetupOIOUBLEDocumentService();
        ...
        // [THEN] The purchase header contains both imported OIOUBL data and manual updates
        OIOUBLStructuredValidations.SetMockCurrencyCode(MockCurrencyCode);
        OIOUBLStructuredValidations.SetMockDate(MockDate);
        OIOUBLStructuredValidations.AssertPurchaseDocument(Vendor."No.", PurchaseHeader, Item);
    end;
    ...
    begin
        ...
        LibraryEDoc.SetupStandardSalesScenario(Customer, EDocumentService, Enum::"E-Document Format"::OIOUBL, Integration);
    ...
    local procedure SetupOIOUBLEDocumentService()
    begin
        EDocumentService."Read into Draft Impl." := "E-Doc. Read into Draft"::OIOUBL;
        EDocumentService.Modify(false);
    end;
