...
codeunit 12216 "Service Declaration Mgt. IT"
{
                                        ...
                                        begin
                                            ...
                                            if not (EntryNo in [0, PrevEntryNo]) then begin
                                                ...
                                                if TempCustLedgEntry.FindSet() then
                                                    repeat
                                                        FilterVATEntryOnCustLedgEntry(VATEntry2, TempCustLedgEntry);
                                                        if VATEntry2.FindSet() then
                                                            repeat
                                                                InsertEUServiceLine(VATEntry2, ServiceDeclarationHeader);
                                        ...
                                        begin
                                            ...
                                            if not (EntryNo in [0, PrevEntryNo]) then begin
                                                ...
                                                if TempVendLedgEntry.FindSet() then
                                                    repeat
                                                        FilterVATEntryOnVendLedgEntry(VATEntry2, TempVendLedgEntry);
                                                        if VATEntry2.FindSet() then
                                                            repeat
                                                                InsertEUServiceLine(VATEntry2, ServiceDeclarationHeader);
                                                            until VATEntry2.Next() = 0;
    ...
    begin
        exit(DataExchangePurchaseXMLTxt);
    end;

    internal procedure GetSaleDataExchDefinition(): Text
    begin
        exit(DataExchangeSaleXMLTxt);
    end;

    internal procedure GetPurchaseCorrectionDataExchDefinition(): Text
    begin
        exit(DataExchangePurchaseCorrectionXMLTxt);
    end;

    internal procedure GetSaleCorrectionDataExchDefinition(): Text
    begin
        exit(DataExchangeSaleCorrectionXMLTxt);
    end;
    ...
    begin
        VATEntryAmount := VATEntry.Base + VATEntry."Nondeductible Base";
        case VATEntry.Type of
            VATEntry.Type::Purchase:
                exit(VATEntryAmount);
            VATEntry.Type::Sale:
