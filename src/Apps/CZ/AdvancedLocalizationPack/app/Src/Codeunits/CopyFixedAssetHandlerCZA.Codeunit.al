...
codeunit 31474 "Copy Fixed Asset Handler CZA"
{
    ...
    begin
        AutoCreateDefaultDim.SetLoadFields("Dimension Code");
        AutoCreateDefaultDim.SetRange("Table ID", Database::"Fixed Asset");
        if AutoCreateDefaultDim.IsEmpty() then
            exit;

        if AutoCreateDefaultDim.FindSet() then
            repeat
                if DefaultDimension.Get(Database::"Fixed Asset", FixedAsset2."No.", AutoCreateDefaultDim."Dimension Code") then
