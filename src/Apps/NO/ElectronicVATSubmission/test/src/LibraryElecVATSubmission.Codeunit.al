codeunit 148130 "Library - Elec. VAT Submission"
{
    ...
    begin
        SetTestToken(OAuth20Setup."Client ID", 'Dummy Test Client ID');
        SetTestToken(OAuth20Setup."Client Secret", 'Dummy Test Client Secret');
        SetTestToken(OAuth20Setup."Access Token", 'Dummy Test Access Token');
        SetTestToken(OAuth20Setup."Refresh Token", 'Dummy Test Refresh Token');
    end;

    local procedure SetTestToken(var TokenKey: Guid; TokenValue: Text)
    begin
