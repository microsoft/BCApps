...
codeunit 11376 "Create DE Currency Ex. Rate"
{
    SingleInstance = true;
    ...
    begin
        ...
        ContosoCurrency.InsertCurrencyExchangeRate(Currency.GBP(), date, 100, 100, 154.8801, 154.8801);

        date := DMY2Date(2, 1, 2013);
        ContosoCurrency.InsertCurrencyExchangeRate(Currency.GBP(), date, 100, 100, 165.793, 165.793);
        ContosoCurrency.InsertCurrencyExchangeRate(Currency.USD(), date, 100, 100, 101.7599, 101.7599);
        ContosoCurrency.InsertCurrencyExchangeRate(Currency.SEK(), date, 100, 100, 15.7473, 15.7473);

        date := DMY2Date(2, 4, 2013);
        ContosoCurrency.InsertCurrencyExchangeRate(Currency.GBP(), date, 100, 100, 159.2372, 159.2372);
        ContosoCurrency.InsertCurrencyExchangeRate(Currency.USD(), date, 100, 100, 105.0287, 105.0287);
        ContosoCurrency.InsertCurrencyExchangeRate(Currency.SEK(), date, 100, 100, 16.2267, 16.2267);
    ...
    begin
        date := CalcDate('<CY - 3Y + 1D>', WorkDate());
        //GBPdate := CalcDate('<CY - 11Y + 2D>', WorkDate());
        //GBPdate1 := CalcDate('<CY - 11Y + 1M+2D>', WorkDate());
