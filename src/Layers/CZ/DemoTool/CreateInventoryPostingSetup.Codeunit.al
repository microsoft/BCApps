codeunit 101110 "Create Inventory Posting Setup"
{
    ...
    begin
        ...
        // NAVCZ
        InventoryPostingSetup.Validate("Consumption Account CZL", '501100');
        if InventoryPostingSetup."Invt. Posting Group Code" = DemoDataSetup.FinishedCode() then begin
            InventoryPostingSetup.Validate("Change In Inv.Of WIP Acc. CZL", '583100');
            InventoryPostingSetup.Validate("Change In Inv.OfProd. Acc. CZL", '583100');
        end else begin
            InventoryPostingSetup.Validate("Change In Inv.Of WIP Acc. CZL", '581100');
            InventoryPostingSetup.Validate("Change In Inv.OfProd. Acc. CZL", '581100');
        end;
        InventoryPostingSetup.Validate("WIP Account", '121100');
        if InventoryPostingSetup."Invt. Posting Group Code" <> DemoDataSetup.ResaleCode() then
            InventoryPostingSetup.Validate("Material Variance Account", '518100')
        else
