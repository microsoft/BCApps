codeunit 101908 "Make Corrections"
{
    ...
    begin
        ...
        case Date of
            CA.AdjustDate(19020501D):
                begin
                    ...
                    CalcAndPostVAT.InitializeRequest(
                      0D, CA.AdjustDate(19020331D), CA.AdjustDate(19020331D), StrSubstNo(VATDocNumber, XVAT1Q, Date2DMY(Date, 3)), // NAVCZ
                      CA.Convert('995780'), false, true); // NAVCZ
            ...
            CA.AdjustDate(19020801D):
                begin
                    ...
                    CalcAndPostVAT.InitializeRequest(
                      0D, CA.AdjustDate(19020630D), CA.AdjustDate(19020630D), StrSubstNo(VATDocNumber, XVAT2Q, Date2DMY(Date, 3)), // NAVCZ
                      CA.Convert('995780'), false, true); // NAVCZ
                    CalcAndPostVAT.UseRequestPage(false);
                end;
            CA.AdjustDate(19021101D):
                begin
                    ...
                    CalcAndPostVAT.InitializeRequest(
                      0D, CA.AdjustDate(19020930D), CA.AdjustDate(19020930D), StrSubstNo(VATDocNumber, XVAT3Q, Date2DMY(Date, 3)), // NAVCZ
                      CA.Convert('995780'), false, true); // NAVCZ
                    CalcAndPostVAT.UseRequestPage(false);
                end;
            else
        ...
        GLEntry.FindLast();
        InsertOtherEntry(
          0, '995310', CalcDate('<+1D+1M>', GLEntry."Posting Date"), DocNo, XVATSettlement, GLEntry.Amount, // NAVCZ
          0, '995780', ''); // NAVCZ
    end;
