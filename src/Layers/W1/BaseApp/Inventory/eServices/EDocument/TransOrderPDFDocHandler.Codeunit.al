...
codeunit 5017 "Trans. Order PDF Doc.Handler" implements IPdfDocumentHandler
{
    var
        TransferOrderLbl: Label 'Transfer Order';

    /// <summary>
    /// Generates a PDF blob for Transfer Order
    /// </summary>
    ...
    begin
        if not TransferHeader.GetBySystemId(DocumentId) then
            exit(false);

        TransferHeader.SetRange("No.", TransferHeader."No.");
        ReportUsage := "Report Selection Usage"::Inv1;
        ReportSelections.GetPdfReportForTable(TempBlob, ReportUsage, TransferHeader, Database::"Transfer Header");
        DocumentMailing.GetAttachmentFileName(Name, TransferHeader."No.", TransferOrderLbl, ReportUsage.AsInteger());
