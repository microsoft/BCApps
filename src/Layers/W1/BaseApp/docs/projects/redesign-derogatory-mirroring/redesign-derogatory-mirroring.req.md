...
- **FR-014**: Deterministic link-owned reversal with authoritative persisted link
    ...
    - **Acceptance Criteria**:
        ...
        - Reversing calculated Depreciation and Derogatory sources produces the same complete linked reversal graph whether the sources share one transaction or use separate transactions; callers invoke reversal once per distinct source transaction and never re-reverse an entry already covered by an earlier transaction reversal.
...
- **NFR-008**: Cross-localization safety sweep
    - **Metric:** Every localization layer that still declares `Is Derogatory` (field 5865) MUST be swept semantically. Active `Gen. Jnl.-Post Line` consumers in APAC, BE, CH, ES, FI, IT, NA, NO, and RU MUST also be tested at runtime to prove they cannot suppress or duplicate mirroring when combined with the centralized W1 workflow and that reversing calculated sources reverses every linked counterpart exactly once regardless of country-specific transaction grouping. Declare-only layers DACH, GB, NL, and SE MUST be verified to have no localization `GenJnlPostLine.Codeunit.al` consumer and to leave the flag dormant after removal of the inherited W1 consumer.
    - **Rationale:** Binding and transaction grouping must not be inferred from W1 behavior alone; the ES posting defect (C-01) and ES same-transaction reversal shape are confirmed examples of localization behavior that requires composed runtime validation.
    - **Testing Approach:** AL semantic definition/reference tracing per layer plus per-layer posting and reversal regressions asserting exactly one total tax-book counterpart and one linked reversal per source. Reversal fixtures MUST capture original source identities before reversal, collect distinct source transaction numbers, invoke reversal once per distinct transaction, and then validate the complete source/counterpart graph; they MUST NOT use post-reversal `FindLast()` selection or conditionally skip a distinct transaction merely because another source was unexpectedly reversed.
...
| AC-021 | Reversing linked normal-book FA or maintenance sources reverses each source's single linked derogatory counterpart exactly once, with the same final reversal graph when multiple sources share a transaction or occupy separate transactions; reversal is invoked once per distinct source transaction. | Automated test in W1 and composed localization projects | FR-014, NFR-008 |
...
| AC-036 | Each localization declaring `Is Derogatory` (APAC, BE, CH, DACH, ES, FI, GB, IT, NA, NL, NO, RU, SE) is swept semantically and at runtime to prove exactly one total counterpart and correct linked reversal under its actual transaction grouping. | Semantic trace / Composed-country posting and reversal regression | NFR-008 |
