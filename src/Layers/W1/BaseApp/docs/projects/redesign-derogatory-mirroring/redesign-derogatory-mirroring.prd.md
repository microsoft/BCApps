...
Implementation notes (2026-08-06): FA and maintenance reversal now perform a dedicated-key lookup by persisted source entry before consulting mutable setup, reject global duplicate links, retain setup-independent marker-gated legacy fallback, and link reversal chains. Source- and tax-book acquisition reversal also reverse their same-book automatic salvage companions before the primary entry so FA consistency and persisted counterpart links remain valid without recursive or duplicate reversal. W1 BaseApp and Fixed Asset tests compile; the original 15 named EPIC-002 tests pass in the local W1 runtime, and the tax-book-first regression compiles.
