...
Corrective remediation (2026-08-17, DME job 3621451): `RunALTests_ES_Bucket1` showed that `ReverseDerogEntryInitAcqCost` still manually reversed the linked derogatory entry after ITEM-008 had already reversed it automatically. The FILE-020 regression now removes that duplicate reversal attempt and explicitly verifies the persisted-link reversal chain, covering TEST-002/TEST-003 without changing product behavior.
...
Corrective remediation (2026-08-17, DME job 3621451): ITEM-026's RU full-copy parity now includes `DepreciationBook.Table.al` relationship validation; the failing RU codeunit 134166 test is the red evidence for this localized product correction.
...
Independent-review remediation (2026-08-13): The post-emission BaseApp observer was tautological and has been removed. `AllLinkageOutcomeCountsAreAvailableForTelemetry` now clears the upgrade tag, binds codeunit 134194 as a manual `Telemetry Logger`, and runs `RunAfterRelationshipTransfer(false)`. The logger's actual `LogMessage` invocation asserts event `0000FRD` and the exact six dimensions `FALinked=1`, `FAAmbiguous=1`, `FAMissing=1`, `MaintenanceLinked=1`, `MaintenanceAmbiguous=1`, and `MaintenanceMissing=1`, then raises the expected sentinel. Removing or changing the production `FeatureTelemetry.LogUsage` call, event ID, or any required dimension therefore fails the test.

AL LSP tracing established the first-party registration constraint: `System Telemetry Logger` automatically claims publisher `Microsoft`, while `Telemetry Loggers Impl.` retains the first logger for a publisher. The narrow internal `ReplaceLoggerForTesting` seam clears the implementation held by the active event sender, restores the scoped publisher, and registers the manual test implementation; `Telemetry Logger Test Scope` in System Application Test Library is the only caller. Current-source AL MCP builds succeeded for System Application, System Application Test Library, FR BaseApp, and FR Tests-Fixed Asset with zero error diagnostics. AL MCP published the FR BaseApp, scoped test helper, and FR Tests-Fixed Asset packages, and the targeted codeunit 134194 method passed 2/2 including the runner entry. The local FR service is on platform 29.0.52663, so publishing the current System Application source itself was blocked by unrelated newer platform symbols (`Agent.Substate`, `Database Index.Index Type`, and `OpenPermissionSetCard`); runtime validation used a server-compatible package containing the same scoped telemetry seam.

The W1 missing-tax-book regression now requires exactly one normal-book acquisition row with the posted amount and a zero source link before requiring zero tax-book rows. Both salvage regressions now require exactly one source salvage row and invoke the unfiltered `VerifyLinkedCounterparts` cardinality check, preventing extra non-salvage tax rows or unmatched rows from passing. Focused runtime passed the three W1 methods in codeunit 134149 (5/5 including runner entries). Assertion-only W1 changes pin already-correct behavior; no failing product behavior was fabricated.

- EPIC-009: Complete semantic, build, and runtime release gates — DONE (local W1 gates green; localization CI rerun dispatched, not claimed green)

| Task | Description | Status | Relevant Files |
|------|-------------|--------|----------------|
| ITEM-022 | Run AL semantic definition/reference tracing for `Is Derogatory`, relationship resolution, public compatibility procedures, and new-entry reversal lookup. Record, per APAC/BE/CH/DACH/ES/FI/FR/GB/IT/NA/NL/NO/RU/SE layer, zero dual-producer paths and zero unmarked heuristic callers in the implementation evidence. | DONE | `src/Layers/W1/BaseApp/FixedAssets/FixedAsset/DerogatoryPostingMgt.Codeunit.al`, `src/Layers/W1/BaseApp/FixedAssets/FixedAsset/FAInsertLedgerEntry.Codeunit.al`, `src/Layers/W1/BaseApp/FixedAssets/FixedAsset/FAJnlPostLine.Codeunit.al` |
| ITEM-023 | After pushing the implementation branch, execute `$Branch = git branch --show-current; gh workflow run --repo microsoft/BCApps --ref $Branch CICD.yaml`. Require the `Apps W1` and `Apps FR` projects selected by their settings to compile BaseApp and Fixed Asset tests, and require the AL-Go `Clean` dimension to compile FR with `CLEAN29`. Record the concrete branch and workflow run URL in implementation evidence. | DONE for prior builds; fresh rerun pending | `.github/workflows/CICD.yaml`, `.github/workflows/_BuildALGoProject.yaml`, `.github/AL-Go-Settings.json`, `build/projects/Apps W1/.AL-Go/settings.json`, `build/projects/Apps FR/.AL-Go/settings.json` |
| ITEM-027 | In the same AL-Go run, require successful project jobs for APAC, BE, CH, DACH, ES, FI, and GB; inherited W1 codeunit 134149 MUST execute in composed APAC/BE/CH/FI projects and the ES-local regression MUST execute. | Builds DONE; runtime CI blocked/pending rerun | `build/projects/Apps APAC/.AL-Go/settings.json`, `build/projects/Apps BE/.AL-Go/settings.json`, `build/projects/Apps CH/.AL-Go/settings.json`, `build/projects/Apps DACH/.AL-Go/settings.json`, `build/projects/Apps ES/.AL-Go/settings.json`, `build/projects/Apps FI/.AL-Go/settings.json`, `build/projects/Apps GB/.AL-Go/settings.json` |
| ITEM-028 | In the same AL-Go run, require successful project jobs for IT, NA, NL, NO, RU, and SE; inherited W1 codeunit 134149 MUST execute in composed NA/NO projects and IT/RU-local regressions MUST execute. | Builds DONE; runtime CI blocked/pending rerun | `build/projects/Apps IT/.AL-Go/settings.json`, `build/projects/Apps NA/.AL-Go/settings.json`, `build/projects/Apps NL/.AL-Go/settings.json`, `build/projects/Apps NO/.AL-Go/settings.json`, `build/projects/Apps RU/.AL-Go/settings.json`, `build/projects/Apps SE/.AL-Go/settings.json` |
| ITEM-024 | Publish the built apps, run all focused posting/setup/depreciation/maintenance/reversal/cancellation/purchase/FR-upgrade/localization suites with 100% pass, inspect representative FA/maintenance/G/L ledgers, and update OpenSpec tasks only after evidence confirms completion. | W1 local DONE; localization CI pending | `src/Layers/W1/Tests/Fixed Asset/ERMDerogatoryDeprPosting.Codeunit.al`, `openspec/changes/redesign-derogatory-mirroring/tasks.md` |

Implementation and validation notes (2026-08-14, resumed after a forced-restart interruption; AL MCP on service `Navision_navagent2`, W1 unless noted): EPIC-009 was recovered from three prior release-gate commits and one interrupted, unpublished test-first pair, then completed for every locally reachable gate.

- Recovered committed release-gate fixes: `a2a1932c` restored the IT `FA Jnl.-Post Batch` `MakeDerogatoryFAJnlLine` compatibility delegate plus its IT regression (FR-021/PAT-001); `b24d5295` added the FR-003 `"Derogatory Source Entry No."`/`"Legacy Derogatory Ambiguous"` fields and `Key14` to the FI `FA Ledger Entry` override and restored the IT `OnPostLinesOnAfterFAJnlPostLine` event; `45e9e12c` added `private/*` to the FR `Clean` CICD dimension so `CLEAN29` compiles on this branch.

- Interrupted test-first pair (ITEM-022 relationship-resolution finding): `DepreciationBook.Table.al` `"Derogatory Calc."`-OnValidate raised a spurious setup error when the field was validated before the record's own `Code` was assigned, because a blank-`Code` `SetRange("Derogatory Calc.", Code)` matched every book with an empty derogatory calculation. The uniqueness check is now guarded by `if Code <> ''`. Test-first proof on the local server: with the pre-fix Base Application published, W1 codeunit 134166 `OnValidateDerogatoryCalcBeforeCodeAssignmentSucceeds` FAILED (1 passed / 2 failed) with "The depreciation book  is an accounting book and cannot be set up as a derogatory depreciation book." raised from `Depreciation Book`(5611).`Derogatory Calc.-OnValidate`; after publishing the fixed Base Application, codeunit 134166 passed 28/28 and the full posting/reversal matrix codeunit 134149 passed 50/50 — proving the fix and no regression. `UTTABFADerogatoryDepr.Codeunit.al` adds the regression.

- ITEM-022 AL semantic sweep (AL MCP symbol relations + per-layer authoritative-source read): across APAC, BE, CH, DACH, ES, FI, FR, GB, IT, NA, NL, NO, RU, SE there are zero dual-producer paths and zero unmarked heuristic reversal callers. W1 `Gen. Jnl.-Post Line` runs no `Is Derogatory` producer; the sole counterpart producer is `FA Jnl.-Post Line`/`Derogatory Posting Mgt.`. APAC/BE/CH/ES/FI/IT/NA/NO override `Gen. Jnl.-Post Line` but reference `Is Derogatory` only in declaration/`GetDerogatorySetup` (no outer producer); DACH/GB/NL/SE have no override; all inherit W1 `FA Insert Ledger Entry`. FR and RU override `FA Insert Ledger Entry` and resolve counterparts/reversals through the `"Derogatory Source Entry No."` key first, using the legacy scan only when `"Legacy Derogatory Ambiguous"` is set. FR's legacy FA-journal producer is gated by both `#if not CLEAN29` and `if not AcceleratedDeprFeature.IsEnabled()` and otherwise routes through central `DerogatoryPostingMgt.MakeDerogatoryJournalLine` — an either/or route (FR-020), not a second producer.

- ITEM-024 supported-localization builds/runtime (local, AL MCP): W1 BaseApp and Tests-Fixed Asset compile with zero diagnostics; W1 codeunits 134166 (28/28) and 134149 (50/50) pass on `Navision_navagent2`. ES BaseApp + Tests-Fixed Asset compose the W1 Base Application change with no derogatory or `Depreciation Book` diagnostics. (The only local ES diagnostic was a stale generated-view artifact — the committed W1 test helper codeunit 144151 `ERM Derog. Feature Cleanup` absent from a partially regenerated local `src/Views/ES` copy; the file is git-tracked under `src/Layers/W1/Tests/Fixed Asset` and composes correctly in a clean AL-Go build.) FR/NL inherit the same Base Application change and were validated by EPIC-003/006/007/008.

- ITEM-023/027/028 CICD gate — EXTERNAL RUNTIME BLOCKER: prior run 31718032964 (https://github.com/microsoft/BCApps/actions/runs/31718032964), dispatched after `45e9e12c`, completed the W1, FR Default, FR Clean (`CLEAN29`), and affected-localization build jobs successfully. Its unit-test jobs then failed with `Run-AlPipeline` "There are test failures!" across countries and buckets because the private-branch runs could not read the required organization secrets (`licenseFileUrl`, `gitSubmodulesToken`, `AZURE_CREDENTIALS`, code-signing). Commit `67a7bd6a` subsequently reverted the temporary `private/*` Clean selection. On 2026-08-18, `9192474e01` was pushed to `private/algladkov/FR-Derogatory-Depreciation-redesign`; the active account successfully dispatched fresh run 32129755257 (https://github.com/microsoft/BCApps/actions/runs/32129755257) for that branch and SHA, while the second available account still returned `HTTP 403: Must have admin rights to Repository`. Initialization passed and all 25 Default jobs were in progress at evidence capture. This records a concrete fresh rerun without claiming green localization runtime or a fresh Clean result.
...
Corrective test-job remediation (2026-08-17, DME job 3621451): the actionable ES codeunit 134149 stale reversal test and RU codeunit 134166 relationship-validation gap were corrected under EPIC-002/ITEM-008 and EPIC-001/ITEM-001 plus EPIC-005/ITEM-026. The ES missing `Derog. Expense Acc. for Debit` failures ran before current HEAD `81a5ff1e23`, which already fixes setup ordering, so they require rerun evidence only. FR Cloud-to-OnPremise violations, CA/SE/US publish timeouts, and the CZ post-test code-coverage exception remain unrelated infrastructure/runtime reruns and caused no code changes.

Final EPIC-009 closure evidence (2026-08-18):

- Fresh AL MCP semantic queries resolved the two W1 `PostDerogatoryCounterpart` overloads, the link-first `InsertFARevEntryForDerog` API, the W1/FR `MakeDerogatoryFAJnlLine` compatibility delegates, and both FR reversal overloads. A current authoritative-source sweep reconfirmed no `Is Derogatory` producer in any APAC/BE/CH/ES/FI/FR/IT/NA/NO/RU general-journal override; DACH/GB/NL/SE have no override. W1 and FR retain marker-gated legacy fallback after dedicated-link lookup, RU has dedicated-link lookup with no heuristic fallback, and all other listed layers inherit W1. The 14-layer zero-dual-producer and zero-unmarked-heuristic result therefore remains valid after the 2026-08-17 remediations.
- The fresh W1 BaseApp plus Tests-Fixed Asset `al_compile` returned zero diagnostics; both projects built, and their packages published to `Navision_navagent2`. The first current-source codeunit 134149 run honestly failed 48/50: `ReverseDerogEntryInitAcqCost` left two rows because the 2026-08-17 test edit stopped reversing the separate normal-book `Derogatory` source after reversing `Depreciation`. No product defect was found. The test now reverses both independent sources and asserts automatic persisted-link reversal for each counterpart. The targeted rerun passed 3/3; full codeunits 134149, 134150, and 134166 passed 50/50, 9/9, and 29/29, covering representative FA, maintenance, G/L, purchase, cancellation, setup, and reversal ledgers.
- The FR local resolver passed all pre-flight checks, but the generated `src/Views/FR/BaseApp` is stale and lacks the authoritative W1 `Finance/SpendRequest` folder (zero files versus 12 AL files in the layer/W1 view). Its compile consequently returned 105 unrelated missing-namespace/object/stale-symbol diagnostics; no FR green result is claimed and generated views were not modified. FR semantic queries still resolved the current compatibility and reversal overloads from the authoritative linked files. Localization compilation/runtime remains assigned to the dispatched AL-Go gate.
