...
- Observation, not changed: the marker-gated legacy reversal fallback deliberately searches every book other than the source book so it still resolves after the relationship is removed (AC-014/AC-019), which makes the following `TestField("Depreciation Book Code", ...)` vacuous. The French code is at parity with the W1 implementation delivered by EPIC-002, so it is left unchanged here.
