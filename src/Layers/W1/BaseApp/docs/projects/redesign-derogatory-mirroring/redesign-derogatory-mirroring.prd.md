...
Implementation notes (2026-08-06): FA and maintenance reversal now perform a dedicated-key lookup by persisted source entry before consulting mutable setup, reject global duplicate links, retain setup-independent marker-gated legacy fallback, and link reversal chains. Direct counterpart reversal preserves its persisted counterpart role even if later setup makes that book a source. Source- and tax-book acquisition reversal resolve the immediately posted automatic salvage companion by its deterministic ledger sequence and validate its complete posting identity before reversal, so shared metadata cannot reverse unrelated companions. W1 BaseApp and Fixed Asset tests compile. AL MCP executed the 16 EPIC-002 methods available in the previously deployed test app successfully, but AL MCP could not publish the current same-version Base Application package. Full runtime execution of all 22 named EPIC-002 methods remains pending after the final publish. The final test updates add transaction boundaries around expected-error rollback checks and allow the intentionally repeated document number in both depreciation books.
