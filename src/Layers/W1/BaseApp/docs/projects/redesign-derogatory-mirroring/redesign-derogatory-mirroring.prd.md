...
Independent-review remediation (2026-08-12): The earlier French telemetry evidence is corrected: `AllLinkageOutcomeCountsAreAvailableForTelemetry` called the two linkage procedures directly and asserted their output variables, so it did not exercise `RunAfterRelationshipTransfer(false)` or observe event `0000FRD`. The test now clears the upgrade tag, runs that upgrade path, and binds a manual subscriber to an internal post-emission observer raised only after the real `FeatureTelemetry.LogUsage` call returns. The subscriber asserts `0000FRD` and the exact six dimensions `FALinked=1`, `FAAmbiguous=1`, `FAMissing=1`, `MaintenanceLinked=1`, `MaintenanceAmbiguous=1`, and `MaintenanceMissing=1`, then raises the expected sentinel. AL LSP identified the repository's `Test Telemetry Logger`/`Telemetry Loggers Test` pattern; an honest first attempt with that sink failed 0/2 because `System Telemetry Logger` owns the first registration for the shared `Microsoft` publisher, so the narrow BaseApp observer is used instead.

The W1 missing-tax-book regression now requires exactly one normal-book acquisition row with the posted amount and a zero source link before requiring zero tax-book rows. Both salvage regressions now require exactly one source salvage row and invoke the unfiltered `VerifyLinkedCounterparts` cardinality check, preventing extra non-salvage tax rows or unmatched rows from passing. AL MCP compiled W1 and FR with zero diagnostics, built and published both Tests-Fixed Asset apps, and built and published the changed FR BaseApp. Focused runtime passed the three W1 methods in codeunit 134149 (5/5 including runner entries) and `AllLinkageOutcomeCountsAreAvailableForTelemetry` in FR codeunit 134194 (2/2 including runner entries). Assertion-only W1 changes pin already-correct behavior; no failing product behavior was fabricated. The earlier statement that EPIC-008 changed only test sources applies to the prior revision, not this post-review telemetry observer.
