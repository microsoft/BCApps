...
scope: current branch through cdf47118491f0f9734638c5d71f7b90850f907a0
...
**Verdict: PARTIALLY_COMPLIANT (82% weighted requirements completion).** The three implementation failures from the 2026-08-18 review and the transaction-topology-dependent reversal test are resolved in current source. No functional requirement remains at `FAIL`, but release readiness remains partial because four required extensibility-event regressions and the exact-current-SHA localization/CLEAN30/runtime gates are not complete.
...
### Test-Creation Completeness at `cdf4711849`

**Source-test verdict: PARTIAL.** The core accounting behavior has the required automated source coverage, but not every test required by the PRD has been created.

| Test area | Source status | Evidence / remaining gap |
|---|---|---|
| Setup, policy, insertion, and link integrity | **PASS IN SOURCE** | Codeunit 134166 contains 27 tests, including first/reassigned/ambiguous relationships, missing/duplicate/invalid links, generated-mirror ineligibility, compatibility delegation, and posted-source amount coverage. Returning FA and maintenance insertion identities and valid links are also exercised by codeunit 134149. |
| Posting paths, depreciation variants, and total-row cardinality | **PASS IN SOURCE** | Codeunit 134149 contains 48 tests covering purchase invoice, general journal, FA journal, maintenance, G/L integration variants, missing tax-book asset, salvage, automatic-only, final/negative, acquisition-cost, generated-mirror containment, and exact total-versus-linked tax-book counts. |
| FA and maintenance reversal | **PASS IN SOURCE** | Missing, duplicate, already-reversed, setup-change/removal, legacy fallback, salvage, direct-counterpart, and reversal-of-reversal cases exist. `ReverseDerogEntryInitAcqCostBody` now captures both original source/counterpart pairs, deduplicates source transaction numbers with a dictionary, reverses each distinct transaction once, and verifies both linked reversal chains at `ERMDerogatoryDeprPosting.Codeunit.al:828-901`. |
| French upgrade | **PASS IN SOURCE** | Codeunit 134194 contains 35 tests covering unique/missing/ambiguous FA and maintenance pairs, crossed reversal chains, reversal-of-reversal, maintenance code, canceled assets, automatic adjustments, prerequisite/tag sequencing, retry/idempotency, telemetry, amount invariance, and corrective rollback. |
| Localization composition | **PASS IN SOURCE; EXECUTION PENDING** | The transaction-neutral W1 tests are inherited by composed country projects; focused ES, IT, RU, and NL regressions cover the known full-copy/declaration differences. The current exact SHA still needs execution in every PRD-listed composed project. |
| Extensibility event ordering and final validation | **PARTIAL: 1 OF 5** | `FinalLinkValidationRunsAfterPostingEvent` covers only `OnPostFixedAssetOnBeforeInsertEntry`. No focused subscriber regression covers `OnBeforeFAJnlPostLine`, `OnBeforeGenJnlPostLine`, `OnBeforePostDeprUntilDate`, or `OnPostMaintenanceOnBeforeInsertEntry`. These four tests are still required by NFR-007 / AC-027 / ITEM-020. |
| Compile, runtime, and ledger/G/L inspection gates | **NOT A TEST-CREATION GAP; EVIDENCE PENDING** | TEST-006/007 require builds, published execution, and representative ledger inspection rather than additional AL test methods. No GitHub Actions run targets `cdf4711849`. |
...
| Reversal test assumes one transaction topology | **RESOLVED IN SOURCE** | Commit `cdf4711849` updates `ReverseDerogEntryInitAcqCostBody` to capture both source/counterpart identities, deduplicate transaction numbers with `Dictionary.Set`, reverse each distinct transaction once, and verify both complete linked reversal chains at `ERMDerogatoryDeprPosting.Codeunit.al:828-901`. | Execute the inherited test in W1 and ES/composed localization shapes. |
| FR-013/020/021, NFR-001/004/005/006/008, ITEM-010/011/012/013/023/024/026/027/028: localization, CLEAN30, API-consumer, and runtime gates | **STILL PARTIAL** | Current HEAD is `cdf4711849`. No GitHub Actions run targets this SHA. Run `32169810643` targeted older SHA `7e53d2c9c1` and completed `failure`; its W1 compile failed on AA0005 before later fixes. DME job 3623139 targeted older SHA `1f44d82357` and found further code and infrastructure failures; later commits address reported code causes, but no final rerun evidence is recorded. Representative cross-country ledger/G/L inspection is also not recorded. | Rerun CI/CD at the exact current HEAD, including W1, FR, FR Clean/CLEAN30, and every concrete country project, then rerun the affected DME suites. Record exact SHA, run/job URLs, focused suite totals, and representative FA/maintenance/G/L ledger evidence. |
...
1. Complete the four missing event-order regressions.
2. Produce a green exact-HEAD W1/FR/CLEAN30/localization build and runtime rerun, including the transaction-neutral reversal test and final ledger/G/L evidence.
3. Make insertion validation explicitly select the dedicated link key and capture representative timing evidence.
