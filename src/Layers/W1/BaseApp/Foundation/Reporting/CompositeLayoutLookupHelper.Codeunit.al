...
codeunit 9665 "Composite Layout Lookup Helper"
{
    ...
    /// <summary>
    /// Filters Tenant Report Layout Cfg to the rows that assign the given part, matching its composite reference in the
    /// column that carries parts of that subtype - Header Part Name for a header/footer, Theme Part Name for a theme.
    /// Returns false for a layout that is neither, in which case no filter is applied and the caller must not read the
    /// record. Use it to list where a part is used: the filter is on the column that holds the part.
    /// </summary>
    internal procedure SetPartAssignmentFilter(var Cfg: Record "Tenant Report Layout Cfg"; PartLayout: Record "Report Layout List"): Boolean
    var
        Composite: Text;
    begin
