...
codeunit 5467 "PDF Document Management"
{
    ...
    var
        IPdfDocumentHandler: Interface IPdfDocumentHandler;
    begin
        IPdfDocumentHandler := DocumentType;
        DocumentFound := IPdfDocumentHandler.GeneratePdfBlobWithDocumentType(DocumentId, DocumentType, TempAttachmentEntityBuffer);
        if not DocumentFound then
            Error(CannotFindDocumentErr, DocumentId);
    end;

    /// <summary>
    /// Inits and adds generated report to the buffer
    /// </summary>
    /// <param name="DocumentId">The printed document</param>
    /// <param name="DocumentType">Document type</param>
    /// <param name="TempBlob">Generated PDF file</param>
    /// <param name="Name">File Name</param>
    /// <param name="TempAttachmentEntityBuffer">Returns buffer containing printed document</param>
    /// <returns>True if TempAttachmentEntityBuffer is inserted, otherwise false.</returns>
    internal procedure AddToTempAttachmentEntityBuffer(DocumentId: Guid; DocumentType: Enum "Attachment Entity Buffer Document Type"; TempBlob: Codeunit "Temp Blob"; Name: Text[250]; var TempAttachmentEntityBuffer: Record "Attachment Entity Buffer" temporary): Boolean
    var
        InStream: InStream;
        OutStream: OutStream;
    begin
        ...
        CopyStream(OutStream, InStream);
        exit(TempAttachmentEntityBuffer.Insert(true));
    end;
}
