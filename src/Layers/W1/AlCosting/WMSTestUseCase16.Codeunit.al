codeunit 103326 "WMS Test Use Case 16"
{
    ...
    begin
        TestscriptMgt.InitializeOutput(CODEUNIT::"WMS Test Use Case 16");
    ...
    begin
        ...
        if LastIteration = '16-1-1-10' then
            exit;

        GlobalPrecondition.SetupLocation('STD', true, false, false, false, 1);

        if LastIteration = '16-1-1-20' then
            exit;

        Bin.Get('WHITE', 'W-07-0002');
        ...
        TestScriptMgmt.InsertAdjmtWhseJnlLine(WhseJnlLine, 'ADJMT', 'DEFAULT', 'WHITE', TestScriptMgmt.GetNextNo(WhseJnlLineNo), 20011126D,
          'C_TEST', '32', 'PICK', 'W-04-0001', 50, 'PCS');

        if LastIteration = '16-1-2-10' then
            exit;

        TestScriptMgmt.WhseJnlPostBatch(WhseJnlLine);
        TestScriptMgmt.CalcWhseAdjustment(ItemJnlLine, 20011125D, 'TCS16-1-2');
        TestScriptMgmt.ItemJnlPostBatch(ItemJnlLine);

        if LastIteration = '16-1-2-20' then
            exit;

        TestScriptMgmt.InsertProdOrder(ProdOrder, 3, 0, 'D_PROD', 100, 'WHITE');
        TestScriptMgmt.InsertProdOrderLine(ProdOrder, ProdOrderLine, 10000, 'D_PROD', 'WHITE', 100, 'PCS');

        if LastIteration = '16-1-2-30' then
            exit;
        ...
        if LastIteration = '16-1-2-40' then
            exit;
        ...
        if LastIteration = '16-1-2-50' then
            exit;
        ...
        if LastIteration = '16-1-2-60' then
            exit;
        // 16-1-3
        TestScriptMgmt.VerifyPostCondition(UseCaseNo, TestCaseNo, 3, LastILENo);

        if LastIteration = '16-1-3-10' then
            exit;
        // 16-1-4
        TestScriptMgmt.PostConsumption('CONSUMP', 'DEFAULT', 10000, 'WHITE', 20011128D, '101001', 'C_TEST', '32', 30, 10000);

        if LastIteration = '16-1-4-10' then
            exit;
        // 16-1-5
        TestScriptMgmt.VerifyPostCondition(UseCaseNo, TestCaseNo, 5, LastILENo);

        if LastIteration = '16-1-5-10' then
            exit;
        // 16-1-6
        TestScriptMgmt.InsertWhseIntPutAwayOrderHead(IntPutAwayHeader, 'WHITE', 'PRODUCTION', 'W-07-0002');
        TestScriptMgmt.InsertWhseIntPutAwayOrderLines(IntPutAwayLine, IntPutAwayHeader, 10000, 'C_TEST', '32', 'WHITE', 'PRODUCTION',
          'W-07-0002', 20, 'PCS');

        if LastIteration = '16-1-6-10' then
            exit;

        TestScriptMgmt.CreaPutAwayFromIntPutAwayOrder(IntPutAwayHeader);

        if LastIteration = '16-1-6-20' then
            exit;
        // 16-1-7
        TestScriptMgmt.VerifyPostCondition(UseCaseNo, TestCaseNo, 7, LastILENo);

        if LastIteration = '16-1-7-10' then
            exit;
        // 16-1-8
        WhseActivLine.Reset();
