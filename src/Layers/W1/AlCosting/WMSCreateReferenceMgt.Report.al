report 103203 "WMS Create Reference Mgt"
{
    ...
    dataset
    {
        dataitem("Integer"; "Integer")
        {
            ...
            begin
                ...
                if SelectProdOrderComp then
                    CreateRefProdOrderComp();
                if SelectResEntry then
                    CreateRefResEntry();
                if SelectITSpec then
                    CreateRefITSpec();
            end;
            ...
            begin
                ...
                OutFile.Create(TemporaryPath + '\WMS_ReferenceManagement.txt');
    ...
    var
        ...
        ItemJnlLineRef: Record "WMS Item Journal Line Ref";
        ...
        ResEntryRef: Record "WMS Reservation Entry Ref";
        ITSpecRef: Record "WMS Tracking Specification Ref";
        ...
        SelectProdOrderComp: Boolean;
        SelectResEntry: Boolean;
        SelectITSpec: Boolean;

    [Scope('OnPrem')]
    procedure CreateRefWhseEntry()
    begin
        CodeunitNo := 103335;
        CodeunitName := 'WMS Warehouse Entry Ref. Mgmt';
        CreateHeader();
    ...
    begin
        ...
        RefTableID := DATABASE::"WMS Warehouse Entry Ref";
        LoopIterations();
    end;
    ...
    begin
        CodeunitNo := 103336;
        CodeunitName := 'WMS ItemLedgEntry Ref. Mgmt';
        CreateHeader();
    ...
    begin
        ...
        RefTableID := DATABASE::"WMS Item Ledger Entry Ref";
        LoopIterations();
    end;
    ...
    begin
        CodeunitNo := 103337;
        CodeunitName := 'WMS Bin Content Ref. Mgmt';
        CreateHeader();
    ...
    begin
        ...
        RefTableID := DATABASE::"WMS Bin Content Ref";
        LoopIterations();
    end;
    ...
    begin
        CodeunitNo := 103338;
        CodeunitName := 'WMS Whse. Activity Ref. Mgmt';
        CreateHeader();
    ...
    begin
        ...
        RefTableID := DATABASE::"WMS Whse. Activity Line Ref";
        LoopIterations();
    end;
    ...
    begin
        ...
        WriteOutFile('    UnitOfMeasure@1013 : Code[10];ZoneCode@1016 : Code[10];');
        WriteOutFile('    BinCode@1017 : Code[20];Quantity@1018 : Decimal;QtyHandled@1019 : Decimal;');
        WriteOutFile('    Cubage@1021 : Decimal;Weight@1022 : Decimal);');
        WriteOutFile('    VAR');
        ...
        WriteOutFile('            UseCaseNo,TestCaseNo,LineNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(WhseActivLine.FIELDNAME("Zone Code"),WhseActivLine."Zone Code",ZoneCode,');
        WriteOutFile('            UseCaseNo,TestCaseNo,LineNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        ...
        WriteOutFile('            UseCaseNo,TestCaseNo,LineNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(WhseActivLine.FIELDNAME(Cubage),WhseActivLine.Cubage,Cubage,');
        WriteOutFile('            UseCaseNo,TestCaseNo,LineNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(WhseActivLine.FIELDNAME(Weight),WhseActivLine.Weight,Weight,');
        WriteOutFile('            UseCaseNo,TestCaseNo,LineNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('        END;');
        WriteOutFile('      END;');
    ...
    begin
        CodeunitNo := 103339;
        CodeunitName := 'WMS WhseJnlLine Ref. Mgmt';
        CreateHeader();
    ...
    begin
        ...
        RefTableID := DATABASE::"WMS Warehouse Journal Line Ref";
        LoopIterations();
    end;
    ...
    begin
        WriteOutFile('    PROCEDURE Test@' + NextProcNo() + '(JnlTmplteName : Code[10];JnlBatchName : Code[10];' +
          ...
          'FromZoneCode : Code[10];FromBCode : Code[20];ToZoneCode : Code[10];' +
          'ToBCode : Code[20];Qty : Decimal;QtyBase : Decimal);');
        WriteOutFile('    VAR');
    ...
    begin
        CodeunitNo := 103340;
        CodeunitName := 'WMS ItemJnlLine Ref. Mgmt';
        CreateHeader();
    ...
    begin
        ...
        RefTableID := DATABASE::"WMS Item Journal Line Ref";
        LoopIterations();
    end;
    ...
    begin
        CodeunitNo := 103341;
        CodeunitName := 'WMS Whse. Rcpt. Line Ref. Mgmt';
        CreateHeader();
    ...
    begin
        ...
        RefTableID := DATABASE::"WMS Warehouse Receipt Line Ref";
        LoopIterations();
    end;
    ...
    begin
        CodeunitNo := 103342;
        CodeunitName := 'WMS Pstd. Rcpt.-Line Ref. Mgmt';
        CreateHeader();
    ...
    begin
        ...
        RefTableID := DATABASE::"WMS Posted Whse. Rcpt Line Ref";
        LoopIterations();
    end;
    ...
    begin
        CodeunitNo := 103343;
        CodeunitName := 'WMS Whse. Shpt.-Line Ref. Mgmt';
        CreateHeader();
    ...
    begin
        ...
        RefTableID := DATABASE::"WMS Whse. Shipment Line Ref";
        LoopIterations();
    end;
    ...
    begin
        CodeunitNo := 103344;
        CodeunitName := 'WMS Pstd. Shpt.-Line Ref. Mgmt';
        CreateHeader();
    ...
    begin
        ...
        RefTableID := DATABASE::"WMS Posted Whse. Shpt Line Ref";
        LoopIterations();
    end;
    ...
    begin
        CodeunitNo := 103345;
        CodeunitName := 'WMS WhseWkshLine Ref. Mgmt';
        CreateHeader();
    ...
    begin
        ...
        RefTableID := DATABASE::"WMS Whse. Worksheet Line Ref";
        LoopIterations();
    end;
    ...
    begin
        WriteOutFile('    PROCEDURE Test@' + NextProcNo() + '(WkshTemplate@1001 : Code[10];LineNo@1004 : Integer;' +
          'ItemNo@1005 : Code[20];VariantCode@1006 : Code[10];UnitofMeasure@1007 : Code[10];FromZoneCode@1012 : Code[10];' +
          'FromBinCode@1013 : Code[20];ToZoneCode@1008 : Code[10];');
    ...
    [Scope('OnPrem')]
    procedure CreateRefProdOrderComp()
    begin
        CodeunitNo := 103346;
        CodeunitName := 'WMS ProdOrderComp Ref. Mgmt';
        CreateHeader();
        CreateVerifyWhseEntry();
        CreateTestWhseEntry();
        CreateFooter();
    ...
    [Scope('OnPrem')]
    procedure CreateVerifyProdOrderComp()
    begin
        Window.Update(1, 'Prod. Order Component');
        ProcedureHeader := StrSubstNo(ProcedureHeaderTmpl, NextProcNo());
        TableText := 'DATABASE::"Prod. Order Component"';
        RefTableID := DATABASE::"WMS Prod. Order Component Ref";
        LoopIterations();
    end;

    [Scope('OnPrem')]
    procedure CreateTestProdOrderComp()
    begin
        WriteOutFile('    PROCEDURE Test@' + NextProcNo() + '(Status : Option;ProdOrderNo : Code[20];ProdOrderLnNo : Integer;' +
          'LnNo : Integer;ItemNo : Code[20];UoM : Code[10];Qty : Decimal;RoutLink : Code[10];Variant : Code[10];' +
          'Location : Code[10];Bin : Code[20];QtyBase : Decimal;ExpQty : Decimal;ExpQtyBase : Decimal;RemainQty : Decimal;' +
          'RemainQtyBase : Decimal;ReservQty : Decimal;ReservQtyBase : Decimal;PickQty : Decimal;PickQtyBase : Decimal;' +
          'QtyPicked : Decimal;QtyPickedBase : Decimal;ActConsump : Decimal;FlushMethod : Option;PlanLvl : Integer;DueDate : Date);');
        WriteOutFile('    VAR');
        WriteOutFile('      ProdOrderComp : Record "Prod. Order Component";');
        WriteOutFile('    BEGIN');
        WriteOutFile('      WITH TestScriptMgmt DO BEGIN');
        WriteOutFile('        EntryNo := EntryNo + Offset;');
        WriteOutFile('        IF NOT ProdOrderComp.GET(Status,ProdOrderNo,ProdOrderLnNo,LnNo") THEN');
        WriteOutFile('          TestTextValue('''',''NOT FOUND'',''EXIST'',UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo)');
        WriteOutFile('        else BEGIN');
        WriteOutFile('          Increase(NoOfRecords);');
        WriteOutFile('          TestTextValue(ProdOrderComp.FIELDNAME("Item No."),ProdOrderComp."Item No.",ItemNo,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(ProdOrderComp.FIELDNAME("Unit of Measure Code"),' +
          'ProdOrderComp."Unit of Measure Code",UoM,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME(Quantity),ProdOrderComp.Quantity,Qty,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(ProdOrderComp.FIELDNAME("Routing Link Code"),' +
          'ProdOrderComp."Routing Link Code",RoutLink,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(ProdOrderComp.FIELDNAME("Variant Code"),ProdOrderComp."Variant Code",Variant,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(ProdOrderComp.FIELDNAME("Location Code"),ProdOrderComp."Location Code",Location,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(ProdOrderComp.FIELDNAME("Bin Code"),ProdOrderComp."Bin Code",Bin,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME("Quantity (Base)"),ProdOrderComp."Quantity (Base)",QtyBase,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME("Expected Quantity"),ProdOrderComp."Expected Quantity",ExpQty,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME("Expected Qty. (Base)"),' +
          'ProdOrderComp."Expected Qty. (Base)",ExpQtyBase,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME("Remaining Quantity"),' +
          'ProdOrderComp."Remaining Quantity",RemainQty,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME("Remaining Qty. (Base)"),' +
          'ProdOrderComp."Remaining Qty. (Base)",RemainQtyBase,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile(
          '          TestNumberValue(ProdOrderComp.FIELDNAME("Reserved Quantity"),ProdOrderComp."Reserved Quantity",ReservQty,'
          );
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME("Reserved Qty. (Base)"),' +
          'ProdOrderComp."Reserved Qty. (Base)",ReserveQtyBase,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME("Pick Qty."),ProdOrderComp."Pick Qty.",PickQty,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile(
          '          TestNumberValue(ProdOrderComp.FIELDNAME("Pick Qty. (Base)"),ProdOrderComp."Pick Qty. (Base)",PickQtyBase,'
          );
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME("Qty. Picked"),ProdOrderComp."Qty. Picked",QtyPicked,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME("Qty. Picked (Base)"),' +
          'ProdOrderComp."Qty. Picked (Base)",QtyPickedBase,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME("Act. Consumption (Qty)"),' +
          'ProdOrderComp."Act. Consumption (Qty)",ActConsump,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile(
          '          TestNumberValue(ProdOrderComp.FIELDNAME("Flushing Method"),ProdOrderComp."Flushing Method",FlushMethod,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ProdOrderComp.FIELDNAME("Planning Level Code"),' +
          'ProdOrderComp."Planning Level Code",PlanLvl,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestDateValue(ProdOrderComp.FIELDNAME("Due Date"),ProdOrderComp."Due Date",DueDate,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('        END;');
        WriteOutFile('      END;');
    ...
    [Scope('OnPrem')]
    procedure CreateRefResEntry()
    begin
        CodeunitNo := 103347;
        CodeunitName := 'WMS Reserv. Entry Ref. Mgmt';
        CreateHeader();
        CreateVerifyResEntry();
        CreateTestResEntry();
        CreateFooter();
    end;

    [Scope('OnPrem')]
    procedure CreateVerifyResEntry()
    begin
        Window.Update(1, 'Reservation Entry');
        ProcedureHeader := StrSubstNo(ProcedureHeaderTmpl, NextProcNo());
        TableText := 'DATABASE::"Reservation Entry"';
        RefTableID := DATABASE::"WMS Reservation Entry Ref";
        LoopIterations();
    end;

    [Scope('OnPrem')]
    procedure CreateTestResEntry()
    begin
        WriteOutFile('    PROCEDURE Test@' + NextProcNo() + '(EntryNo : Integer;LocationCode : Code[10];ShptDate : Date;' +
          'SrcType : Integer;SourceID : Code[20];SrcRefNo : Integer;ItemNo : Code[20];VariantCode : Code[10];' +
          'Quantity : Decimal;QuantityBase : Decimal;QtyToHandleBase : Decimal;QtyToInvoiceBase : Decimal;QtyInvoicedBase : Decimal;' +
          'SerialNo : Code[20];LotNo : Code[20];Positive : Text[3]);');
        WriteOutFile('    VAR');
        WriteOutFile('      ResEntry : Record "Reservation Entry";');
        WriteOutFile('      Positiv : Boolean;');
        WriteOutFile('    BEGIN');
        WriteOutFile('      WITH TestScriptMgmt DO BEGIN');
        WriteOutFile('        EntryNo := EntryNo + Offset;');
        WriteOutFile('        EVALUATE(Positiv,Positive);');
        WriteOutFile('        IF NOT ResEntry.GET(EntryNo,Positiv) THEN');
        WriteOutFile('          TestTextValue('''',''NOT FOUND'',''EXIST'',UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo)');
        WriteOutFile('        else BEGIN');
        WriteOutFile('          Increase(NoOfRecords);');
        WriteOutFile('          TestTextValue(ResEntry.FIELDNAME("Location Code"),ResEntry."Location Code",LocationCode,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestDateValue(ResEntry.FIELDNAME("Shipment Date"),ResEntry."Shipment Date",ShptDate,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ResEntry.FIELDNAME("Source Type"),ResEntry."Source Type",SrcType,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(ResEntry.FIELDNAME("Source ID"),ResEntry."Source ID",SourceID,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ResEntry.FIELDNAME("Source Ref. No."),ResEntry."Source Ref. No.",SrcRefNo,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(ResEntry.FIELDNAME("Item No."),ResEntry."Item No.",ItemNo,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(ResEntry.FIELDNAME("Variant Code"),ResEntry."Variant Code",VariantCode,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ResEntry.FIELDNAME(Quantity),ResEntry.Quantity,Quantity,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ResEntry.FIELDNAME("Quantity (Base)"),ResEntry."Quantity (Base)",QuantityBase,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ResEntry.FIELDNAME("Qty. to Handle (Base)"),' +
          'ResEntry."Qty. to Handle (Base)",QtyToHandleBase,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ResEntry.FIELDNAME("Qty. to Invoice (Base)"),' +
          'ResEntry."Qty. to Invoice (Base)",QtyToInvoiceBase,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(ResEntry.FIELDNAME("Quantity Invoiced (Base)"),' +
          'ResEntry."Quantity Invoiced (Base)",QtyInvoicedBase,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(ResEntry.FIELDNAME("Serial No."),ResEntry."Serial No.",SerialNo,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(ResEntry.FIELDNAME("Lot No."),ResEntry."Lot No.",LotNo,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('        END;');
        WriteOutFile('      END;');
    ...
    [Scope('OnPrem')]
    procedure CreateRefITSpec()
    begin
        CodeunitNo := 103348;
        CodeunitName := 'WMS Track. Spec. Ref. Mgmt';
        CreateHeader();
        CreateVerifyITSpec();
        CreateTestITSpec();
        CreateFooter();
    ...
    [Scope('OnPrem')]
    procedure CreateVerifyITSpec()
    begin
        Window.Update(1, 'Tracking Specifikation');
        ProcedureHeader := StrSubstNo(ProcedureHeaderTmpl, NextProcNo());
        TableText := 'DATABASE::"Tracking Specification"';
        RefTableID := DATABASE::"WMS Tracking Specification Ref";
        LoopIterations();
    end;

    [Scope('OnPrem')]
    procedure CreateTestITSpec()
    begin
        WriteOutFile('    PROCEDURE Test@' + NextProcNo() + '(EntryNo : Integer;LocationCode : Code[10];QtyHandledBase : Decimal;' +
          'QtyToHandle : Decimal;SrcType : Integer;SourceID : Code[20];SrcRefNo : Integer;ItemNo : Code[20];VariantCode : Code[10];' +
          'QuantityBase : Decimal;QtyToInvoiceBase : Decimal;QtyToInvoice : Decimal;SerialNo : Code[20];LotNo : Code[20]);');
        WriteOutFile('    VAR');
        WriteOutFile('      TrackSpec : Record "Tracking Specification";');
        WriteOutFile('    BEGIN');
        WriteOutFile('      WITH TestScriptMgmt DO BEGIN');
        WriteOutFile('        EntryNo := EntryNo + Offset;');
        WriteOutFile('        IF NOT TrackSpec.GET(EntryNo) THEN');
        WriteOutFile('          TestTextValue('''',''NOT FOUND'',''EXIST'',UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo)');
        WriteOutFile('        else BEGIN');
        WriteOutFile('          Increase(NoOfRecords);');
        WriteOutFile('          TestTextValue(TrackSpec.FIELDNAME("Location Code"),TrackSpec."Location Code",LocationCode,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(TrackSpec.FIELDNAME("Quantity Handled (Base)"),' +
          'TrackSpec."Quantity Handled (Base)",QtyHandledBase,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(TrackSpec.FIELDNAME("Qty. to Handle"),TrackSpec."Qty. to Handle",QtyToHandle,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(TrackSpec.FIELDNAME("Source Type"),TrackSpec."Source Type",SrcType,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(TrackSpec.FIELDNAME("Source ID"),TrackSpec."Source ID",SourceID,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(TrackSpec.FIELDNAME("Source Ref. No."),TrackSpec."Source Ref. No.",SrcRefNo,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(TrackSpec.FIELDNAME("Item No."),TrackSpec."Item No.",ItemNo,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(TrackSpec.FIELDNAME("Variant Code"),TrackSpec."Variant Code",VariantCode,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(TrackSpec.FIELDNAME("Quantity (Base)"),TrackSpec."Quantity (Base)",QuantityBase,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestNumberValue(TrackSpec.FIELDNAME("Qty. to Invoice"),TrackSpec."Qty. to Invoice",QtyToInvoice,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(TrackSpec.FIELDNAME("Serial No."),TrackSpec."Serial No.",SerialNo,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('          TestTextValue(TrackSpec.FIELDNAME("Lot No."),TrackSpec."Lot No.",LotNo,');
        WriteOutFile('            UseCaseNo,TestCaseNo,EntryNo,TableID,IterationNo);');
        WriteOutFile('          Increase(NoOfFields);');
        WriteOutFile('        END;');
        WriteOutFile('      END;');
    ...
    begin
        ...
        WriteOutFile('  ' + Format(LBracket) + '');
        WriteOutFile('    VAR');
        WriteOutFile('      TestScriptMgmt : Codeunit "WMS TestscriptManagement";');
    ...
    begin
        ...
        UseCases.Reset();
        UseCases.SetRange("Project Code", 'WMS');
    ...
    begin
        ...
        TempRefData.SetRange("Project Code", 'WMS');
        ...
        if WhseActivLineRef.Find('-') then
            repeat
                ...
                InsertTempRefData(
                  ...
                  FormatInteger(WhseActivLineRef."Activity Type".AsInteger()) + Comma +
                  ...
                  FormatInteger(WhseActivLineRef."Action Type".AsInteger()) + Comma +
                  ...
                  FormatText(WhseActivLineRef."Zone Code") + Comma +
                  ...
                  FormatDecimal(WhseActivLineRef.Cubage) + Comma +
                  FormatDecimal(WhseActivLineRef.Weight)
                  );
    ...
    [Scope('OnPrem')]
    procedure ProcessProdOrderCompRef()
    begin
        RefTableID := DATABASE::"WMS Prod. Order Component Ref";
        EntryNo := 0;
        ...
        if ProdOrderCompRef.Find('-') then
            repeat
                EntryNo := EntryNo + 1;
                InsertTempRefData(
                  RefTableID, ProdOrderCompRef."Use Case No.", ProdOrderCompRef."Test Case No.", ProdOrderCompRef."Iteration No.", EntryNo,
                  FormatInteger(ProdOrderCompRef.Status) + Comma +
                  FormatText(ProdOrderCompRef."Prod. Order No.") + Comma +
                  FormatInteger(ProdOrderCompRef."Prod. Order Line No.") + Comma +
                  FormatInteger(ProdOrderCompRef."Line No.") + Comma +
                  FormatText(ProdOrderCompRef."Item No.") + Comma +
                  FormatText(ProdOrderCompRef."Unit of Measure Code") + Comma +
                  FormatDecimal(ProdOrderCompRef.Quantity) + Comma +
                  FormatText(ProdOrderCompRef."Routing Link Code") + Comma +
                  FormatText(ProdOrderCompRef."Variant Code") + Comma +
                  FormatText(ProdOrderCompRef."Location Code") + Comma +
                  FormatText(ProdOrderCompRef."Bin Code") + Comma +
                  FormatDecimal(ProdOrderCompRef."Quantity (Base)") + Comma +
                  FormatDecimal(ProdOrderCompRef."Expected Quantity") + Comma +
                  FormatDecimal(ProdOrderCompRef."Expected Qty. (Base)") + Comma +
                  FormatDecimal(ProdOrderCompRef."Remaining Quantity") + Comma +
                  FormatDecimal(ProdOrderCompRef."Remaining Qty. (Base)") + Comma +
                  FormatDecimal(ProdOrderCompRef."Reserved Quantity") + Comma +
                  FormatDecimal(ProdOrderCompRef."Reserved Qty. (Base)") + Comma +
                  FormatDecimal(ProdOrderCompRef."Pick Qty.") + Comma +
                  FormatDecimal(ProdOrderCompRef."Pick Qty. (Base)") + Comma +
                  FormatDecimal(ProdOrderCompRef."Qty. Picked") + Comma +
                  FormatDecimal(ProdOrderCompRef."Qty. Picked (Base)") + Comma +
                  FormatDecimal(ProdOrderCompRef."Act. Consumption (Qty)") + Comma +
                  FormatInteger(ProdOrderCompRef."Flushing Method") + Comma +
                  FormatInteger(ProdOrderCompRef."Planning Level Code") + Comma +
                  FormatDate(ProdOrderCompRef."Due Date")
                  );
            until ProdOrderCompRef.Next() = 0;
    end;

    [Scope('OnPrem')]
    procedure ProcessResEntryRef()
    begin
        RefTableID := DATABASE::"WMS Reservation Entry Ref";
        EntryNo := 0;
        ResEntryRef.Reset();
        if ResEntryRef.Find('-') then
            repeat
                EntryNo := EntryNo + 1;
                InsertTempRefData(
                  RefTableID, ResEntryRef."Use Case No.", ResEntryRef."Test Case No.", ResEntryRef."Iteration No.", EntryNo,
                  FormatInteger(ResEntryRef."Entry No.") + Comma +
                  FormatText(ResEntryRef."Location Code") + Comma +
                  FormatDate(ResEntryRef."Shipment Date") + Comma +
                  FormatInteger(ResEntryRef."Source Type") + Comma +
                  FormatText(ResEntryRef."Source ID") + Comma +
                  FormatInteger(ResEntryRef."Source Ref. No.") + Comma +
                  FormatText(ResEntryRef."Item No.") + Comma +
                  FormatText(ResEntryRef."Variant Code") + Comma +
                  FormatDecimal(ResEntryRef.Quantity) + Comma +
                  FormatDecimal(ResEntryRef."Quantity (Base)") + Comma +
                  FormatDecimal(ResEntryRef."Qty. to Handle (Base)") + Comma +
                  FormatDecimal(ResEntryRef."Qty. to Invoice (Base)") + Comma +
                  FormatDecimal(ResEntryRef."Quantity Invoiced (Base)") + Comma +
                  FormatText(ResEntryRef."Serial No.") + Comma +
                  FormatText(ResEntryRef."Lot No.") + Comma +
                  Apostrophe + Format(ResEntryRef.Positive) + Apostrophe
                  );
            until ResEntryRef.Next() = 0;
    end;

    [Scope('OnPrem')]
    procedure ProcessITSpecRef()
    begin
        RefTableID := DATABASE::"WMS Tracking Specification Ref";
        EntryNo := 0;
        ITSpecRef.Reset();
        if ITSpecRef.Find('-') then
            repeat
                EntryNo := EntryNo + 1;
                InsertTempRefData(
                  RefTableID, ITSpecRef."Use Case No.", ITSpecRef."Test Case No.", ITSpecRef."Iteration No.", EntryNo,
                  FormatInteger(ITSpecRef."Entry No.") + Comma +
                  FormatText(ITSpecRef."Location Code") + Comma +
                  FormatDecimal(ITSpecRef."Quantity Handled (Base)") + Comma +
                  FormatDecimal(ITSpecRef."Qty. to Handle") + Comma +
                  FormatInteger(ITSpecRef."Source Type") + Comma +
                  FormatText(ITSpecRef."Source ID") + Comma +
                  FormatInteger(ITSpecRef."Source Ref. No.") + Comma +
                  FormatText(ITSpecRef."Item No.") + Comma +
                  FormatText(ITSpecRef."Variant Code") + Comma +
                  FormatDecimal(ITSpecRef."Quantity (Base)") + Comma +
                  FormatDecimal(ITSpecRef."Qty. to Invoice") + Comma +
                  FormatDecimal(ITSpecRef."Qty. to Invoice (Base)") + Comma +
                  FormatText(ITSpecRef."Serial No.") + Comma +
                  FormatText(ITSpecRef."Lot No.")
                  );
            until ITSpecRef.Next() = 0;
    end;
