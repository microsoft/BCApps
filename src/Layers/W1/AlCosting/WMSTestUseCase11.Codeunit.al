codeunit 103321 "WMS Test Use Case 11"
{
    ...
    begin
        TestscriptMgt.InitializeOutput(CODEUNIT::"WMS Test Use Case 11");
    ...
    begin
        ...
        if LastIteration = '11-1-1-10' then
            exit;
        ...
        if LastIteration = '11-1-1-20' then
            exit;

        GlobalPrecondition.SetupLocation('STD', false, false, false, false, 0);

        if LastIteration = '11-1-1-30' then
            exit;
        GetLastILENo();
        ...
        TestScriptMgmt.InsertAdjmtWhseJnlLine(WhseJnlLine, 'ADJMT', 'DEFAULT', 'WHITE', TestScriptMgmt.GetNextNo(WhseJnlLineNo), 20011126D,
          'A_TEST', '', 'PICK', 'W-01-0001', 21, 'PCS');
        TestScriptMgmt.InsertAdjmtWhseJnlLine(WhseJnlLine, 'ADJMT', 'DEFAULT', 'WHITE', TestScriptMgmt.GetNextNo(WhseJnlLineNo), 20011126D,
          'A_TEST', '', 'QC', 'W-10-0001', 1, 'PCS');
        TestScriptMgmt.InsertAdjmtWhseJnlLine(WhseJnlLine, 'ADJMT', 'DEFAULT', 'WHITE', TestScriptMgmt.GetNextNo(WhseJnlLineNo), 20011126D,
          'A_TEST', '', 'SHIP', 'W-09-0001', 3, 'PCS');
        TestScriptMgmt.InsertAdjmtWhseJnlLine(WhseJnlLine, 'ADJMT', 'DEFAULT', 'WHITE', TestScriptMgmt.GetNextNo(WhseJnlLineNo), 20011126D,
          'A_TEST', '11', 'PICK', 'W-01-0002', 21, 'PCS');
        TestScriptMgmt.InsertAdjmtWhseJnlLine(WhseJnlLine, 'ADJMT', 'DEFAULT', 'WHITE', TestScriptMgmt.GetNextNo(WhseJnlLineNo), 20011126D,
        ...
        if LastIteration = '11-1-2-10' then
            exit;

        TestScriptMgmt.WhseJnlPostBatch(WhseJnlLine);

        if LastIteration = '11-1-2-20' then
            exit;
        // 11-1-3
        TestScriptMgmt.VerifyPostCondition(UseCaseNo, TestCaseNo, 3, LastILENo);

        if LastIteration = '11-1-3-10' then
            exit;
        // 11-1-4
        TestScriptMgmt.CalcWhseAdjustment(ItemJnlLine, 20011125D, 'TCS11-1-4');
        ...
        TestScriptMgmt.InsertItemJnlLine(ItemJnlLine, 'ITEM', 'DEFAULT', TestScriptMgmt.GetNextNo(ItemJnlLineNo), 20011125D,
          ItemJnlLine."Entry Type"::Purchase, 'TCS11-1-2', 'A_TEST', '', 'BLUE', '', 5, 'PCS', 10, 0);
        TestScriptMgmt.InsertItemJnlLine(ItemJnlLine, 'ITEM', 'DEFAULT', TestScriptMgmt.GetNextNo(ItemJnlLineNo), 20011125D,
          ItemJnlLine."Entry Type"::Purchase, 'TCS11-1-2', 'A_TEST', '', 'GREEN', '', 10, 'PCS', 10, 0);
        TestScriptMgmt.InsertItemJnlLine(ItemJnlLine, 'ITEM', 'DEFAULT', TestScriptMgmt.GetNextNo(ItemJnlLineNo), 20011125D,
          ItemJnlLine."Entry Type"::Purchase, 'TCS11-1-2', 'A_TEST', '11', 'BLUE', '', 10, 'PCS', 15, 0);
        TestScriptMgmt.InsertItemJnlLine(ItemJnlLine, 'ITEM', 'DEFAULT', TestScriptMgmt.GetNextNo(ItemJnlLineNo), 20011125D,
          ItemJnlLine."Entry Type"::Purchase, 'TCS11-1-2', 'A_TEST', '11', 'GREEN', '', 8, 'PCS', 15, 0);
        TestScriptMgmt.InsertItemJnlLine(ItemJnlLine, 'ITEM', 'DEFAULT', TestScriptMgmt.GetNextNo(ItemJnlLineNo), 20011125D,
          ItemJnlLine."Entry Type"::Purchase, 'TCS11-1-2', 'A_TEST', '11', 'BLUE', '', 1, 'PALLET', 100, 0);
        TestScriptMgmt.InsertItemJnlLine(ItemJnlLine, 'ITEM', 'DEFAULT', TestScriptMgmt.GetNextNo(ItemJnlLineNo), 20011125D,
          ItemJnlLine."Entry Type"::Purchase, 'TCS11-1-2', 'A_TEST', '11', 'GREEN', '', 2, 'PALLET', 100, 0);

        if LastIteration = '11-1-4-10' then
            exit;

        TestScriptMgmt.ItemJnlPostBatch(ItemJnlLine);

        if LastIteration = '11-1-4-20' then
            exit;
        // 11-1-5
        TestScriptMgmt.VerifyPostCondition(UseCaseNo, TestCaseNo, 5, LastILENo);

        if LastIteration = '11-1-5-10' then
            exit;
        // 11-1-6
        ...
        if LastIteration = '11-1-6-10' then
            exit;
        // 11-1-7
        TestScriptMgmt.VerifyPostCondition(UseCaseNo, TestCaseNo, 7, LastILENo);

        if LastIteration = '11-1-7-10' then
            exit;
        // 11-1-8
        TestScriptMgmt.ModifyWhseJnlLine(WhseJnlLine."Journal Template Name", WhseJnlLine."Journal Batch Name", 'WHITE', 10000, 19);
        TestScriptMgmt.ModifyWhseJnlLine(WhseJnlLine."Journal Template Name", WhseJnlLine."Journal Batch Name", 'WHITE', 20000, 20);
        TestScriptMgmt.ModifyWhseJnlLine(WhseJnlLine."Journal Template Name", WhseJnlLine."Journal Batch Name", 'WHITE', 30000, 1);
        TestScriptMgmt.ModifyWhseJnlLine(WhseJnlLine."Journal Template Name", WhseJnlLine."Journal Batch Name", 'WHITE', 40000, 2);

        if LastIteration = '11-1-8-10' then
            exit;

        TestScriptMgmt.WhseJnlPostBatch(WhseJnlLine);

        if LastIteration = '11-1-8-20' then
            exit;
        // 11-1-9
        TestScriptMgmt.VerifyPostCondition(UseCaseNo, TestCaseNo, 9, LastILENo);

        if LastIteration = '11-1-9-10' then
            exit;
        // 11-1-10
        Clear(ItemJnlLine);
        ...
        CalcQtyOnHand.InitializeRequest(20011129D, ItemJnlLine."Document No.", false, false);
        CalcQtyOnHand.UseRequestPage(false);
        CalcQtyOnHand.RunModal();
        Clear(CalcQtyOnHand);

        if LastIteration = '11-1-10-10' then
            exit;
        // 11-1-11
        TestScriptMgmt.VerifyPostCondition(UseCaseNo, TestCaseNo, 11, LastILENo);

        if LastIteration = '11-1-11-10' then
            exit;
        // 11-1-12
        ...
        CalcQtyOnHand.SetItemJnlLine(ItemJnlLine);
        Item.Reset();
        Item.SetRange("Location Filter", 'WHITE');
        CalcQtyOnHand.SetTableView(Item);
        CalcQtyOnHand.InitializeRequest(20011129D, ItemJnlLine."Document No.", true, false);
        CalcQtyOnHand.UseRequestPage(false);
        CalcQtyOnHand.RunModal();
        Clear(CalcQtyOnHand);

        if LastIteration = '11-1-12-10' then
            exit;
        // 11-1-13
        TestScriptMgmt.VerifyPostCondition(UseCaseNo, TestCaseNo, 13, LastILENo);

        if LastIteration = '11-1-13-10' then
            exit;
        // 11-1-14
        TestScriptMgmt.ItemJnlPostBatch(ItemJnlLine);

        if LastIteration = '11-1-14-10' then
            exit;
        // 11-1-15
        TestScriptMgmt.VerifyPostCondition(UseCaseNo, TestCaseNo, 15, LastILENo);

        if LastIteration = '11-1-15-10' then
            exit;
    end;
