...
codeunit 137079 "SCM Production Order III"
{
    ...
    [Test]
    procedure PostPurchaseReturnOrderForSubcontracting()
    var
        Item: Record Item;
        WorkCenter: Record "Work Center";
        ProductionOrder: Record "Production Order";
    ...
    begin
        // [FEATURE] [Subcontracting] [Purchase Return Order]
        // [SCENARIO 647380] A purchase return order for subcontracting can be posted.
        Initialize();

        // [GIVEN] A posted subcontracting purchase order linked to a released production order.
        CreateProductionItem(Item, '');
        CreateRoutingAndUpdateItemSubc(Item, WorkCenter, true);
        CreateAndRefreshReleasedProductionOrder(ProductionOrder, Item."No.", LibraryRandom.RandInt(10), '', '');
        CalculateSubcontractOrder(WorkCenter);
        ...
        // [GIVEN] A purchase return order copied from the posted subcontracting invoice.
        PurchInvHeader.SetRange("Order No.", PurchaseOrderNo);
        PurchInvHeader.FindFirst();
        LibraryPurchase.CreatePurchHeader(
            PurchaseReturnOrder, PurchaseReturnOrder."Document Type"::"Return Order", WorkCenter."Subcontractor No.");
        PurchaseReturnOrder.Validate("Vendor Cr. Memo No.", LibraryUtility.GenerateGUID());
        PurchaseReturnOrder.Modify(true);
        LibraryPurchase.CopyPurchaseDocument(
            PurchaseReturnOrder, "Purchase Document Type From"::"Posted Invoice", PurchInvHeader."No.", false, false);
        PurchaseReturnOrderNo := PurchaseReturnOrder."No.";
        PurchaseReturnLine.SetRange("Document Type", PurchaseReturnLine."Document Type"::"Return Order");
        PurchaseReturnLine.SetRange("Document No.", PurchaseReturnOrderNo);
        PurchaseReturnLine.SetRange(Type, PurchaseReturnLine.Type::Item);
        PurchaseReturnLine.FindFirst();
        PurchaseReturnLine.TestField("Operation No.", PurchaseLine."Operation No.");
        PurchaseReturnLine.TestField("Work Center No.", PurchaseLine."Work Center No.");
        Assert.IsTrue(
            PurchaseReturnLine.IsSubcontractingCreditMemo(),
            'The purchase return line must be recognized as a subcontracting credit document.');

        // [WHEN] The subcontracting purchase return order is shipped and invoiced.
        LibraryPurchase.PostPurchaseDocument(PurchaseReturnOrder, true, true);

        // [THEN] A return shipment and purchase credit memo are posted for the return order.
        ReturnShipmentHeader.SetRange("Return Order No.", PurchaseReturnOrderNo);
