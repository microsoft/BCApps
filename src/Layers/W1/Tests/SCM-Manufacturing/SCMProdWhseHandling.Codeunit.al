...
codeunit 137298 "SCM Prod. Whse. Handling"
{
    ...
    begin
        // [FEATURE] [AI test 0.4]
        // [SCENARIO 647854] Calling DeleteProdOrderRelations directly (bypassing Delete/OnDelete, as first-party cleanup code such as Subcontracting does) is still blocked when a component has picked quantity not yet consumed.
        Initialize();
        ...
        // [WHEN] DeleteProdOrderRelations is called directly, without going through Delete/OnDelete.
        asserterror ProductionOrder.DeleteProdOrderRelations();

        // [THEN] Error is raised because picked quantity has not been consumed, and the components are not removed.
        Assert.ExpectedError(CannotDeleteWithPickedQtyErr);
        ProdOrderComponent.SetRange(Status, ProductionOrder.Status);
        ProdOrderComponent.SetRange("Prod. Order No.", ProductionOrder."No.");
        Assert.RecordIsNotEmpty(ProdOrderComponent);
