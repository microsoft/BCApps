...
codeunit 137914 "SCM Whse.-Asm. To Order"
{
    ...
    [Test]
    [HandlerFunctions('AssignLotToAssemblyHeader,MessageHandler')]
    procedure InvtPickForATOLineBeforeNormalItemStillValidatesNormalLine()
    var
        AsmHeader: Record "Assembly Header";
        AssemblyBin: Record Bin;
        ComponentBin: Record Bin;
    ...
    begin
        // [FEATURE] [AI test 0.4]
        // [SCENARIO 640983] When the Assemble-to-Order line is the FIRST line on the Sales Order, posting the Inventory Pick still runs item-tracking bin-content validation for the following normal tracked line (the ATO line is skipped per-line, not via a line-order-dependent early exit).
        Initialize();
        ...
        // [GIVEN] Sales Order with line 10000 = output item "O" (ATO, first) from the empty bin "B" and line 20000 = normal item "N" from bin "E"
        LibrarySales.CreateSalesHeader(SalesHeader, SalesHeader."Document Type"::Order, '');
        PostponeShptDateforAssemblyLeadTime(SalesHeader);
        SalesHeader.Validate("Location Code", Location.Code);
        SalesHeader.Modify(true);
        ...
        // [GIVEN] Lots assigned on the Inventory Pick lines for the output item and the normal item
        SetLotOnInvtPickLine(SalesHeader."No.", OutputItem."No.", OutputLotNo);
        SetLotOnInvtPickLine(SalesHeader."No.", NormalItem."No.", NormalItemLotNo);
        ...
        // [THEN] The normal tracked line was still validated and picked: a warehouse entry for the normal item with its lot "DEF" exists, proving the line following the first ATO line was not skipped
        WarehouseEntry.SetRange("Item No.", NormalItem."No.");
        WarehouseEntry.SetRange("Lot No.", NormalItemLotNo);
        Assert.IsFalse(WarehouseEntry.IsEmpty(), 'The normal tracked line following the ATO line should have been validated and picked.');
