codeunit 134605 "Test Report Layout Selection"
{
    ...
    [Test]
    [Scope('OnPrem')]
    procedure ShowReportBlocksDownloadingAnotherUsersReport()
    var
        ReportInbox: Record "Report Inbox";
    begin
        // [FEATURE] [Report Inbox]
        // [SCENARIO 640047] A user cannot download a report that was generated by another user
        ReportInbox.DeleteAll();

        // [GIVEN] A report inbox entry generated by another user
        ReportInbox."User ID" := CopyStr(LibraryUtility.GenerateGUID(), 1, MaxStrLen(ReportInbox."User ID"));
        ReportInbox."Output Type" := ReportInbox."Output Type"::PDF;
        ReportInbox."Report ID" := 134600;
        ReportInbox.Insert(true);

        // [WHEN] The current user tries to download (show) the report
        asserterror ReportInbox.ShowReport();

        // [THEN] An error is raised stating the report belongs to another user
        Assert.ExpectedError(CannotAccessOtherUsersReportErr);
    end;

    [Test]
    [HandlerFunctions('ReportIsEmptyMsgHandler')]
    [Scope('OnPrem')]
    procedure ShowReportAllowsDownloadingOwnReport()
    var
        ReportInbox: Record "Report Inbox";
    begin
        // [FEATURE] [Report Inbox]
        // [SCENARIO 640047] A user can download a report that they generated
        ReportInbox.DeleteAll();

        // [GIVEN] A report inbox entry generated by the current user
        ReportInbox."User ID" := CopyStr(UserId(), 1, MaxStrLen(ReportInbox."User ID"));
        ReportInbox."Output Type" := ReportInbox."Output Type"::PDF;
        ReportInbox."Report ID" := 134600;
        ReportInbox.Insert(true);

        // [WHEN] The current user downloads (shows) the report
        // [THEN] No access error is raised - the empty report message is shown instead (handled)
        ReportInbox.ShowReport();
    end;

    [Test]
    [Scope('OnPrem')]
    procedure ReportInboxPageShowsOnlyCurrentUsersReports()
    var
        ReportInbox: Record "Report Inbox";
        ReportInboxTestPage: TestPage "Report Inbox";
    ...
    begin
        // [FEATURE] [Report Inbox] [UI]
        // [SCENARIO 640047] The Report Inbox page only shows reports generated by the current user
        ReportInbox.DeleteAll();

        // [GIVEN] A report inbox entry for the current user
        ReportInbox."User ID" := CopyStr(UserId(), 1, MaxStrLen(ReportInbox."User ID"));
        ReportInbox."Output Type" := ReportInbox."Output Type"::PDF;
        ReportInbox."Report ID" := 134600;
        ReportInbox.Insert(true);

        // [GIVEN] A report inbox entry for another user
        Clear(ReportInbox);
        ReportInbox."User ID" := CopyStr(LibraryUtility.GenerateGUID(), 1, MaxStrLen(ReportInbox."User ID"));
        ReportInbox."Output Type" := ReportInbox."Output Type"::PDF;
        ReportInbox."Report ID" := 134600;
        ...
        // [WHEN] The Report Inbox page is opened
        ReportInboxTestPage.OpenView();

        // [THEN] Only the current user's report is visible
        VisibleRecordCount := 0;
        if ReportInboxTestPage.First() then
            repeat
                Assert.AreEqual(UserId(), ReportInboxTestPage."User ID".Value, 'Report Inbox should only show the current user''s reports');
                VisibleRecordCount += 1;
            until not ReportInboxTestPage.Next();
        Assert.AreEqual(1, VisibleRecordCount, 'Report Inbox should show exactly one report for the current user');
