...
codeunit 137917 "SCM Item Appln. Entry Perf"
{
    ...
    begin
        ...
        // because no forward edge exists in that direction — the application is O_P1 -> C_P2, not
        // the reverse. A cache that conflated directions would risk flipping this answer. Guards
        // the per-direction cache-keying requirement called out in spec §4.2.
        Initialize();
        LibraryInventory.CreateItem(Item);
        ...
        // [THEN] Not reachable in this direction — assertion is FALSE
        Assert.IsFalse(
    ...
    begin
        ...
        // design rule in spec §4.3 that the inner cache gate is placed AFTER the TrackChain block.
        Initialize();
        NoOfOutputs := 3;
        ...
        // [THEN] Chain contains at least one reachable ILE — the consumption that the first P1 output
        ...
        // [WHEN] Second walk (exercises any cross-call state issues; stability is the real G3 guard —
    ...
    begin
        ...
        // instance — this test binds a fresh local instance and primes its TriggerReentry flag.
        Initialize();
        LibraryInventory.CreateItem(Item);
        Qty := 3;
        ...
        // [WHEN] Baseline call — no subscriber bound
        AnswerNoReentry := ItemApplicationEntry.CheckIsCyclicalLoop(OutputItemLedgerEntry, ConsumptionItemLedgerEntry);

        // [WHEN] Reentrant call — bind subscriber instance and prime its trigger flag, so the
    ...
    begin
        ...
        // outputs — for each closed P1 output, it descends into the corresponding P2 consumption
    ...
    begin
        ...
        // all N postings grow as N*(2N+1) = O(N^2) without the Prod cache vs O(N) with it —
    ...
    begin
        ...
        // consumes N — those N open Item1 outputs are the supply for the measurement postings.
