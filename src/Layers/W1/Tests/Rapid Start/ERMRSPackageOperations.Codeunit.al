codeunit 136603 "ERM RS Package Operations"
{
    ...
    var
        ...
        PriceListNosNotSetupErr: Label 'Price list nos. should be set up.', Locked = true;
        PriceListHeaderCodeAutoGeneratedErr: Label 'Price list header code should be auto-generated from no. series.', Locked = true;
        ExportedOrdinalErr: Label 'Option and enum values must be exported as language-independent ordinals.', Locked = true;
    ...
    begin
        ...
        // [THEN] the exported values are language-independent ordinals, not localized captions
        VerifyOptionsAndEnumsExportedAsOrdinals(FilePath, ConfigPackage.Code);
    ...
    [Test]
    [Scope('OnPrem')]
    procedure ApplyPriceListHeaderPackageWithEmptyCodeNoDuplicate()
    var
        ConfigPackage: Record "Config. Package";
        ConfigPackageRecord: Record "Config. Package Record";
        ConfigPackageTable: Record "Config. Package Table";
    ...
    begin
        // [FEATURE] [AI test]
        // [SCENARIO 617675] Applying Config Package with Price List Header where Code is empty creates one record
        Initialize();

        // [GIVEN] A valid No. Series is set up for Sales Price List
        SalesReceivablesSetup.Get();
        Assert.AreNotEqual('', SalesReceivablesSetup."Price List Nos.", PriceListNosNotSetupErr);

        // [GIVEN] A Config Package "CP" with table "Price List Header" containing one record with empty Code
        LibraryRapidStart.CreatePackage(ConfigPackage);
        LibraryRapidStart.CreatePackageTable(ConfigPackageTable, ConfigPackage.Code, Database::"Price List Header");
        LibraryRapidStart.CreatePackageRecord(ConfigPackageRecord, ConfigPackage.Code, Database::"Price List Header", 1);
        LibraryRapidStart.CreatePackageFieldData(ConfigPackageRecord, PriceListHeader.FieldNo(Code), '');
        LibraryRapidStart.CreatePackageFieldData(ConfigPackageRecord, PriceListHeader.FieldNo(Description), 'Test Price List');
        LibraryRapidStart.CreatePackageFieldData(ConfigPackageRecord, PriceListHeader.FieldNo("Source Group"), Format(PriceListHeader."Source Group"::Customer));
        LibraryRapidStart.CreatePackageFieldData(ConfigPackageRecord, PriceListHeader.FieldNo("Source Type"), Format(PriceListHeader."Source Type"::"All Customers"));
        LibraryRapidStart.CreatePackageFieldData(ConfigPackageRecord, PriceListHeader.FieldNo("Price Type"), Format(PriceListHeader."Price Type"::Sale));
        LibraryRapidStart.CreatePackageFieldData(ConfigPackageRecord, PriceListHeader.FieldNo("Amount Type"), Format(PriceListHeader."Amount Type"::Price));
        LibraryRapidStart.CreatePackageFieldData(ConfigPackageRecord, PriceListHeader.FieldNo(Status), Format(PriceListHeader.Status::Draft));

        // [WHEN] The package "CP" is applied
        LibraryRapidStart.ApplyPackage(ConfigPackage, true);

        // [THEN] No errors occurred during application
        VerifyNoConfigPackageErrors(ConfigPackage.Code);

        // [THEN] Exactly one Price List Header "PLH" record exists with Description 'Test Price List'
        PriceListHeader.SetRange(Description, 'Test Price List');
        Assert.RecordCount(PriceListHeader, 1);

        // [THEN] The created record "PLH" has a non-empty auto-generated Code
        PriceListHeader.FindFirst();
        Assert.AreNotEqual('', PriceListHeader.Code, PriceListHeaderCodeAutoGeneratedErr);
    ...
    local procedure VerifyOptionsAndEnumsExportedAsOrdinals(FilePath: Text; PackageCode: Code[20])
    var
        OptionAndEnumRS: Record OptionAndEnumRS;
        XMLDOMManagement: Codeunit "XML DOM Management";
        XMLDocument: DotNet XmlDocument;
        OptionNodes: DotNet XmlNodeList;
        EnumNodes: DotNet XmlNodeList;
        ExpectedOptionOrdinal: array[3] of Integer;
        ExpectedEnumOrdinal: array[3] of Integer;
        i: Integer;
    begin
        ExpectedOptionOrdinal[1] := OptionAndEnumRS.OptionField::Zero;
        ExpectedOptionOrdinal[2] := OptionAndEnumRS.OptionField::One;
        ExpectedOptionOrdinal[3] := OptionAndEnumRS.OptionField::Two;
        ExpectedEnumOrdinal[1] := OptionAndEnumRS.EnumField::Eight.AsInteger();
        ExpectedEnumOrdinal[2] := OptionAndEnumRS.EnumField::Nine.AsInteger();
        ExpectedEnumOrdinal[3] := OptionAndEnumRS.EnumField::Ten.AsInteger();

        XMLDOMManagement.LoadXMLDocumentFromFile(FilePath, XMLDocument);
        XMLDOMManagement.FindNodes(
          XMLDocument.DocumentElement,
          '//' + GetOptionAndEnumRSElementName(PackageCode, OptionAndEnumRS.FieldNo(OptionField)), OptionNodes);
        XMLDOMManagement.FindNodes(
          XMLDocument.DocumentElement,
          '//' + GetOptionAndEnumRSElementName(PackageCode, OptionAndEnumRS.FieldNo(EnumField)), EnumNodes);

        Assert.AreEqual(ArrayLen(ExpectedOptionOrdinal), OptionNodes.Count, ExportedOrdinalErr);
        Assert.AreEqual(ArrayLen(ExpectedEnumOrdinal), EnumNodes.Count, ExportedOrdinalErr);

        for i := 1 to ArrayLen(ExpectedOptionOrdinal) do begin
            Assert.AreEqual(Format(ExpectedOptionOrdinal[i]), OptionNodes.Item(i - 1).InnerText, ExportedOrdinalErr);
            Assert.AreEqual(Format(ExpectedEnumOrdinal[i]), EnumNodes.Item(i - 1).InnerText, ExportedOrdinalErr);
        end;
    end;

    local procedure GetOptionAndEnumRSElementName(PackageCode: Code[20]; FieldNo: Integer): Text
    var
        ConfigPackageField: Record "Config. Package Field";
    begin
        ConfigPackageField.Get(PackageCode, Database::OptionAndEnumRS, FieldNo);
        exit(ConfigPackageField.GetValidatedElementName());
    end;
