...
page 507 "Blanket Sales Order"
{
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Deactivate recurrence"; Rec."Deactivate recurrence")
                {
                    ApplicationArea = Suite;
                    ToolTip = 'Specifies if you do not want the recurring group for the sales header.';
                }
            }
        ...
        area(processing)
        {
            ...
            action(MakeOrder)
            {
                ...
                var
                    ...
                    ApprovalsMgmt: Codeunit "Approvals Mgmt.";
                begin
                    if Rec."Recurring Group Code" = '' then begin
                        if ApprovalsMgmt.PrePostApprovalCheckSales(Rec) then
                            CODEUNIT.Run(CODEUNIT::"Blnkt Sales Ord. to Ord. (Y/N)", Rec)
                    end else begin
                        if not Confirm(Text10600, false) then
                            exit;
                        SalesHeader := Rec;
    ...
    var
        ...
        Text10600: Label 'This is a Recurring Order.\Do you whish to create a Sales Order?';
