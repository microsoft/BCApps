codeunit 110000 "Interface Basis Data"
{
    ...
    begin
        ...
        if PostingDate = CA.AdjustDate(19021231D) then begin
            CloseSettledVATPeriods(CA.AdjustDate(19041231D), false);
            ...
            CloseSettledVATPeriods(Today, true);
        end;
    ...
    local procedure CloseSettledVATPeriods(BeforePostingDate: Date; Closed: Boolean)
    var
        SettledVATPeriod: Record "Settled VAT Period";
    begin
        SettledVATPeriod.SetFilter(Year, '<%1', Date2DMY(BeforePostingDate, 3));
        SettledVATPeriod.ModifyAll(Closed, Closed);
    end;
