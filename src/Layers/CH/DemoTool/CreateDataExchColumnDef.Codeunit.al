codeunit 118230 "Create Data Exch. Column Def"
{
    ...
    local procedure CheckSwissSEPACAMTFiles(Path: Text)
    begin
        if not Exists(Path + '\SEPA CAMT 053-02.xml') then
            Error(NoFileErr, Path);
        if not Exists(Path + '\SEPA CAMT 053-04.xml') then
            Error(NoFileErr, Path);
        if not Exists(Path + '\SEPA CAMT 054.xml') then
            Error(NoFileErr, Path);
    end;
}
