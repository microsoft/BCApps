...
#pragma warning restore AA0074
            ...
            SalesHeader."Document Type"::Quote:
                case SalesReceivablesSetup."Archive Quotes" of
                    SalesReceivablesSetup."Archive Quotes"::Always:
                        ArchSalesNoConfirmWithQuote(SalesHeader, SalesHeaderArchive."Quote Status"::Deleted);
                    SalesReceivablesSetup."Archive Quotes"::Question:
                        ArchSalesWithQuote(SalesHeader, SalesHeaderArchive."Quote Status"::Deleted);
                end;
    ...
    begin
        ...
        ArchSalesWithQuote(SalesHeader, ArchiveSalesHeader."Quote Status"::" ");
    end;
    ...
    begin
        ...
        if IsHandled then
            exit;
        ArchSalesNoConfirmWithQuote(SalesHeader, ArchiveSalesHeader."Quote Status"::" ");
    end;

    [Scope('OnPrem')]
    procedure ArchSalesWithQuote(var SalesHeader: Record "Sales Header"; ArchiveType: Option)
    var
        ConfirmManagement: Codeunit "Confirm Management";
    begin
        if ConfirmManagement.GetResponseOrDefault(
        ...
        then begin
            ArchSalesNoConfirmWithQuote(SalesHeader, ArchiveType);
    ...
    [Scope('OnPrem')]
    procedure ArchSalesNoConfirmWithQuote(var SalesHeader: Record "Sales Header"; ArchiveType: Option)
    var
        ...
        DocType: Integer;
    begin
        if not (SalesHeader."Document Type" in [SalesHeader."Document Type"::Quote, SalesHeader."Document Type"::Order]) then begin
            StoreSalesDocument(SalesHeader, false);
            exit;
        end;

        ArchivedSalesHeader.LockTable();
        StoreSalesDocument(SalesHeader, true);
        ArchivedSalesHeader.SetRange("Document Type", SalesHeader."Document Type");
        ArchivedSalesHeader.SetRange("No.", SalesHeader."No.");

        ArchivedSalesHeader.ModifyAll("Quote Status", ArchivedSalesHeader."Quote Status"::Deleted);

        ArchivedSalesHeader.SetRange("Doc. No. Occurrence", SalesHeader."Doc. No. Occurrence");
        ArchivedSalesHeader.FindLast();
        ArchivedSalesHeader."Quote Status" := ArchiveType;
        ArchivedSalesHeader."Filed Date" := Today;
        ArchivedSalesHeader."Filed User ID" := UserId;
        ArchivedSalesHeader.Modify();

        SalesSetup.Get();
        if SalesSetup."Automatic Interaction Log" then begin
            case SalesHeader."Document Type" of
                SalesHeader."Document Type"::Quote:
                    DocType := 1;
                SalesHeader."Document Type"::Order:
            ...
            if SalesHeader."Bill-to Contact No." <> '' then
                SegManagement.LogDocument(
                  DocType, SalesHeader."No.", SalesHeader."Doc. No. Occurrence",
