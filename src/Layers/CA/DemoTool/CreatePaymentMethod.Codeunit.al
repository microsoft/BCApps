codeunit 101289 "Create Payment Method"
{
    ...
    begin
        ...
        InsertData(XBANKDOMTxt, XBankDomTransferTxt, "Payment Method"."Bal. Account Type"::"G/L Account", '', XBankDataConvPmtLineDefnTxt);
        InsertData(XBANKINTTxt, XBankIntTransferTxt, "Payment Method"."Bal. Account Type"::"G/L Account", '', XBankDataConvPmtLineDefnTxt);
        InsertData(XCASH, XCashpayment, "Payment Method"."Bal. Account Type"::"G/L Account", GetGLAccNo.Cash(), '');
        InsertData(XCHECK, XCheckpayment, "Payment Method"."Bal. Account Type"::"G/L Account", '', '');
        InsertData(XACCOUNT, XPaymentonaccount, "Payment Method"."Bal. Account Type"::"G/L Account", '', '');
        InsertData(XINTERCOM, XIntercompanypayment, "Payment Method"."Bal. Account Type"::"G/L Account", '', '');
        InsertData(XCARD, XCardpayment, "Payment Method"."Bal. Account Type"::"G/L Account", '', '');
    ...
    local procedure InsertTranslation(PaymentMethodCode: Code[10]; LanguageCode: Code[10]; DescriptionTranslation: Text[100])
    var
        PaymentmethodTranslation: Record "Payment Method Translation";
    begin
        PaymentmethodTranslation."Payment Method Code" := PaymentMethodCode;
        PaymentmethodTranslation."Language Code" := LanguageCode;
        PaymentmethodTranslation.Description := DescriptionTranslation;
        if PaymentmethodTranslation.Insert() then;
    end;

    local procedure InsertTranslations()
    begin
        InsertTranslation(XGIRO, FRCTxt, 'Virement giro');
        InsertTranslation(XBANK, FRCTxt, 'Virement bancaire');
        InsertTranslation(XBANK, FRCTxt, 'Virement bancaire');
        InsertTranslation(XBANKDOMTxt, FRCTxt, 'Conversion des données bancaires pour les banques nationales');
        InsertTranslation(XBANKINTTxt, FRCTxt, 'Conversion des données bancaires pour les banques internationales');
        InsertTranslation(XCASH, FRCTxt, 'Paiement au comptant');
        InsertTranslation(XCHECK, FRCTxt, 'Paiement par chèque');
        InsertTranslation(XACCOUNT, FRCTxt, 'Acompte');
        InsertTranslation(XINTERCOM, FRCTxt, 'Paiement inter-compagnie');
        InsertTranslation(XCARD, FRCTxt, 'Paiement par carte');
        InsertTranslation(XMultiple, FRCTxt, 'Diverses methodées de paiement');
    end;
}
