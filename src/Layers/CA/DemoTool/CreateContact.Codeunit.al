codeunit 101550 "Create Contact"
{
    ...
    procedure ModifyContacts()
    var
        Contact: Record Contact;
    begin
        // First, set the companies
        Contact.SetFilter("Country/Region Code", '=CA');
        Contact.Find('-');
        repeat
            if Contact.County in ['NS', 'ON', 'PQ'] then
                Contact."Territory Code" := 'ECAN';
            if Contact.County in ['BC', 'AB'] then
                Contact."Territory Code" := 'WCAN';
            Contact.Modify();
        until Contact.Next() = 0;
        // Now, do the subbies.
        Contact.SetRange("Country/Region Code");
        Contact.SetFilter("Company No.", 'CT000001 | CT000049');
        Contact.Find('-');
        repeat
            if Contact."Country/Region Code" <> 'CA' then begin
                Contact."Country/Region Code" := 'CA';
                Contact."Territory Code" := 'ECAN';
            end;
        until Contact.Next() = 0;
    end;
