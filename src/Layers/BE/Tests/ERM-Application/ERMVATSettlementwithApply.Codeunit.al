codeunit 134008 "ERM VAT Settlement with Apply"
{
    ...
    begin
        ...
        RunCalcAndPostVATSettlement(GenJournalLine, DocumentNo);
        ...
        GeneralLedgerSetup.Validate("Journal Templ. Name Mandatory", false);
    ...
    begin
        ...
        if GenJnlBatch."Posting No. Series" <> '' then
            if ModifySeries then
                exit(NoSeries.GetNextNo(GenJnlBatch."Posting No. Series"))
            else
                exit(NoSeries.PeekNextNo(GenJnlBatch."Posting No. Series"));

        GenJnlBatch.TestField("No. Series");
        if ModifySeries then
            exit(NoSeries.GetNextNo(GenJnlBatch."No. Series"))
        else
    ...
    begin
        SourceCodeSetup.Get();
        GLEntry.SetRange("Document No.", DocumentNo);
        GLEntry.SetRange("Source Code", SourceCodeSetup."VAT Settlement");
        GLEntry.FindSet();
        repeat
