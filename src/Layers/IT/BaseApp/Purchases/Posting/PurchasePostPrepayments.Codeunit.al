...
#pragma warning restore AA0074
    ...
    begin
        ...
        if Abs(TotalPrepmtInvLineBuffer."Amount Incl. VAT") <> PurchHeader."Check Total" then
            Error(Text1130004, Abs(TotalPrepmtInvLineBuffer."Amount Incl. VAT"), PurchHeader."Check Total");

        PurchHeader."Check Total" := 0;
        // Post vendor entry
        Window.Update(4, 1);
    ...
    begin
        ...
        PurchHeader.TestField("Operation Occurred Date");
        PurchHeader.TestField("Operation Type");
        GLSetup.GetRecordOnce();
        ...
        if PurchHeader."Applies-to Doc. No." <> '' then
            if PurchHeader."Applies-to Occurrence No." = 0 then
                Error(Text1130007, PurchHeader.FieldCaption("Applies-to Occurrence No."));
    ...
    begin
        ...
        if HasReverseChargeVAT(PurchHeader) and (PurchHeader."Reverse Sales VAT No." = '') then
            if PurchHeader."Reverse Sales VAT No. Series" <> '' then begin
    ...
    begin
        if GLSetup."Journal Templ. Name Mandatory" then begin
            ...
            PurchHeader."Prepayment No." := NoSeriesCodeunit.GetNextNo(GenJournalTemplate."Posting No. Series", PurchHeader."Posting Date");
            ModifyHeader := true;
        end else begin
            ...
            NoSeries.Get(PurchHeader."Prepayment No. Series");
            NoSeries.TestField("Date Order");
            PurchHeader."Prepayment No." := NoSeriesCodeunit.GetNextNo(PurchHeader."Prepayment No. Series", PurchHeader."Posting Date");
            ModifyHeader := true;
        end
    end;
    ...
    begin
        ...
        if GLSetup."Journal Templ. Name Mandatory" then begin
            ...
            GenJournalTemplate.TestField("Posting No. Series");
            PurchHeader."Prepmt. Cr. Memo No." :=
                NoSeriesCodeunit.GetNextNo(GenJournalTemplate."Posting No. Series", PurchHeader."Posting Date");
            ModifyHeader := true;
        end else begin
            ...
            NoSeries.Get(PurchHeader."Prepmt. Cr. Memo No. Series");
            NoSeries.TestField("Date Order");
            PurchHeader."Prepmt. Cr. Memo No." :=
                NoSeriesCodeunit.GetNextNo(PurchHeader."Prepmt. Cr. Memo No. Series", PurchHeader."Posting Date");
            ModifyHeader := true;
        end;
        ...
        if FromPurchLine.Find('-') then begin
            ...
            if PurchSetup."Invoice Rounding" then begin
                ...
                if InitInvoiceRoundingLine(PurchHeader, TotalAmt, InvRoundingPurchLine) then begin
                    ...
                    ToPurchLine."Prepayment Deductible %" := ToPurchLine."Deductible %";
                    ToPurchLine.Insert();
                end;
            end;
        end;
        ...
        if PurchLine.Find('-') then
            repeat
                if PrepmtAmount(PurchLine, DocumentType) <> 0 then begin
                    if PurchHeader."Document Type" in [PurchHeader."Document Type"::Order, PurchHeader."Document Type"::Invoice, PurchHeader."Document Type"::"Credit Memo"] then begin
                        VATPostingSetup.Get(
    ...
    begin
        ...
        PrepmtInvLineBuf."G/L Account No." := GetPrepmtAccNo(PurchLine."VAT Bus. Posting Group", PurchLine."VAT Prod. Posting Group");
        ...
        PrepmtInvLineBuf."Gen. Bus. Posting Group" := PurchLine."Gen. Bus. Posting Group";
        PrepmtInvLineBuf."VAT Bus. Posting Group" := PurchLine."VAT Bus. Posting Group";
        PrepmtInvLineBuf."VAT Prod. Posting Group" := PurchLine."VAT Prod. Posting Group";
        if VATPostingSetup."EU Service" then begin
            PrepmtInvLineBuf."Service Tariff No." := PurchLine."Service Tariff No.";
            PrepmtInvLineBuf."Transport Method" := PurchLine."Transport Method";
        end;
        PrepmtInvLineBuf."Prepmt. CM Refers to Period" := PurchLine."Prepmt. CM Refers to Period";
    ...
    begin
        if InitInvoiceRoundingLine(PurchHeader, TotalPrepmtInvLineBuf."Amount Incl. VAT", PurchLine) then begin
            ...
            PrepmtInvLineBuf.CopyFromPurchLine(PurchLine);
            PrepmtInvLineBuf."Gen. Bus. Posting Group" := PurchHeader."Gen. Bus. Posting Group";
            PrepmtInvLineBuf."VAT Bus. Posting Group" := PurchHeader."VAT Bus. Posting Group";
    ...
    begin
        ...
        GenJnlLine."Deductible %" := TotalPrepmtInvLineBuffer."Deductible %";
    ...
    begin
        ...
        PurchaseLine.SetRange("VAT Calculation Type", PurchaseLine."VAT Calculation Type"::"Reverse Charge VAT");
    ...
    begin
        ...
        PurchInvLine."Deductible %" := PrepmtInvLineBuffer."Deductible %";
    ...
    begin
        ...
        PurchCrMemoLine."Deductible %" := PrepmtInvLineBuffer."Deductible %";
        ...
        PurchCrMemoLine."Prepmt. CM Refers to Period" := PrepmtInvLineBuffer."Prepmt. CM Refers to Period";
    ...
    begin
        ...
        PostedPaymentLines."Due Date" := PurchHeader."Prepayment Due Date";
        PostedPaymentLines."Discount %" := PurchHeader."Prepmt. Payment Discount %";
        PostedPaymentLines."Pmt. Discount Date" := PurchHeader."Prepmt. Pmt. Discount Date";
    ...
    begin
        ...
        PaymentLines."Due Date" := PurchHeader."Prepayment Due Date";
        PaymentLines."Discount %" := PurchHeader."Prepmt. Payment Discount %";
        PaymentLines."Pmt. Discount Date" := PurchHeader."Prepmt. Pmt. Discount Date";
        PaymentLines.Insert();
    end;
