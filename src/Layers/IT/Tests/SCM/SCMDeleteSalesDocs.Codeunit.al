codeunit 137208 "SCM Delete Sales Docs"
{
    ...
    [Test]
    [HandlerFunctions('ConfirmHandler,SalesShipReportHandler')]
    [Scope('OnPrem')]
    procedure DeletePostedShipment()
    var
        SalesHeader: Record "Sales Header";
    begin
        asserterror DeletePostedDoc(SalesHeader."Document Type"::Order);
    end;

    [Test]
    [HandlerFunctions('ConfirmHandler,SalesInvReportHandler')]
    [Scope('OnPrem')]
    procedure DeletePostedInvoice()
    var
        SalesHeader: Record "Sales Header";
    begin
        asserterror DeletePostedDoc(SalesHeader."Document Type"::Invoice);
    end;

    [Test]
    [HandlerFunctions('ConfirmHandler,RetRcptReportHandler')]
    [Scope('OnPrem')]
    procedure DeletePostedRetReceipt()
    var
        SalesHeader: Record "Sales Header";
    begin
        asserterror DeletePostedDoc(SalesHeader."Document Type"::"Return Order");
    end;

    [Test]
    [HandlerFunctions('ConfirmHandler,CrMemoReportHandler')]
    [Scope('OnPrem')]
    procedure DeletePostedCrMemo()
    var
        SalesHeader: Record "Sales Header";
    begin
        asserterror DeletePostedDoc(SalesHeader."Document Type"::"Credit Memo");
    end;
    ...
    begin
        ...
        for Index := 1 to ArrayLen(PostedShipmentNos) do
            LibraryReportDataset.AssertElementWithValueExists('No_SalesShipHdr', PostedShipmentNos[Index]);
    ...
    [Test]
    [Scope('OnPrem')]
    [HandlerFunctions('ConfirmHandler,SalesShipmentXmlReportHandler')]
    ...
    begin
        ...
        for Index := 1 to ArrayLen(PostedShipmentNos) do
            LibraryXPathXMLReader.VerifyNodeValueByXPathWithIndex(
                '//ReportDataSet/DataItems/DataItem/Columns/Column[@name="No_SalesShipHdr"]',
