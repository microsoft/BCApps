codeunit 134130 "ERM Reverse And VAT Settlement"
{
    ...
    begin
        ...
        for i := 1 to ArrayLen(VATEntry) do begin
            LibraryXPathXMLReader.GetNodeByElementNameByIndex('/DataSet/Result/ProdGroup1_VATPostingSetup', Node, (i * 2) - 2);
            Node := Node.ParentNode;
            LibraryXPathXMLReader.VerifyNodeValueFromParentNode(Node, 'ProdGroup1_VATPostingSetup', VATEntry[i]."VAT Bus. Posting Group");
            LibraryXPathXMLReader.VerifyNodeValueFromParentNode(Node, 'ProdGroup2_VATPostingSetup', VATEntry[i]."VAT Prod. Posting Group");
            LibraryXPathXMLReader.VerifyNodeValueFromParentNode(Node, 'EntryNo_VatEntry', Format(VATEntry[i]."Entry No."));

            LibraryXPathXMLReader.GetNodeByElementNameByIndex('/DataSet/Result/ProdGroup1_VATPostingSetup', Node, (i * 2) - 1);
            Node := Node.ParentNode;
            LibraryXPathXMLReader.VerifyNodeValueFromParentNode(Node, 'ProdGroup1_VATPostingSetup', VATEntry[i]."VAT Bus. Posting Group");
            LibraryXPathXMLReader.VerifyNodeValueFromParentNode(Node, 'ProdGroup2_VATPostingSetup', VATEntry[i]."VAT Prod. Posting Group");
