...
#pragma warning restore AA0074
    ...
    begin
        if GenJnlPosting then begin
            ...
            ShouldCheckNoOfDepreciationDays :=
                (GenJnlLine."FA Posting Type" <> GenJnlLine."FA Posting Type"::Depreciation) and
                (GenJnlLine."FA Posting Type" <> GenJnlLine."FA Posting Type"::"Custom 1") and
                (GenJnlLine."No. of Depreciation Days" <> 0) and
                (GenJnlLine."FA Posting Type" <> GenJnlLine."FA Posting Type"::Derogatory);
            ...
            if GLSetup."Enable Russian Accounting" and (GenJnlLine."FA Posting Type" = GenJnlLine."FA Posting Type"::Disposal) and (not GenJnlLine."System-Created Entry") then
                GenJnlLine.TestField(Amount, 0);
        end;
        ...
        if not GenJnlPosting then begin
            ...
            ShouldCheckNoOfDepreciationDays :=
                (FAJnlLine."FA Posting Type" <> FAJnlLine."FA Posting Type"::Depreciation) and
                (FAJnlLine."FA Posting Type" <> FAJnlLine."FA Posting Type"::"Custom 1") and
                (FAJnlLine."No. of Depreciation Days" <> 0) and
    ...
    begin
        ...
        if GenJnlPosting then begin
            if GenJnlLine."Prepmt. Diff. Appln. Entry No." <> 0 then
                exit;
            if GenJnlLine."VAT Allocation Type" = GenJnlLine."VAT Allocation Type"::Charge then
                exit;
            if GenJnlLine."System-Created Entry" then
                if (GenJnlLine."FA Posting Type" = GenJnlLine."FA Posting Type"::"Acquisition Cost") and (not GenJnlLine."FA Reclassification Entry") then begin
                    FADeprBook.CalcFields("Initial Acquisition Cost");
    ...
    begin
        ...
        then begin
            ...
            if TaxRegisterSetup."Calculate TD for each FA" and DeprBook."Control FA Acquis. Cost" then
                GenJnlLine.TestField("Tax Difference Code");
        end;
        if GenJnlLine."FA Error Entry No." <> 0 then begin
            FALedgerEntry.Get(GenJnlLine."FA Error Entry No.");
            GenJnlLine.TestField("Tax Difference Code", FALedgerEntry."Tax Difference Code");
        end;
    end;
