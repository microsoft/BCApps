...
#pragma warning restore AA0074
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            UnmarkDeprBonusBaseEntries(FAJnlLine."Depr. Bonus", FAJnlLine."FA Posting Date");
        end;
    ...
    begin
        ...
        then begin
            ...
            if DisposalType = DisposalType::ErrorDisposal then
                ...
                FALedgEntry."Disposal Entry No." := -1;
            end;
            for i := 1 to j do
...
#pragma warning restore AA0210
    ...
    begin
        ...
        FAInsertLedgEntry.SetOrgGenJnlLine(false);
        FALedgEntry."Automatic Entry" := true;
        InitialCorrection := FALedgEntry.Correction;
        ...
        if DisposalType in [DisposalType::ErrorDisposal, DisposalType::LastErrorDisposal] then begin
            ...
            ResultOnDisposal := 2;
            for i := 2 to ArrayLen(EntryNumbers) do
                if EntryNumbers[i] <> 0 then begin
                    FALedgEntry.Amount := EntryAmounts[i];
                    FALedgEntry."Entry No." := EntryNumbers[i];
                    FALedgEntry."FA Posting Category" := CalculateDisposal.SetFALedgerPostingCategory(i);
