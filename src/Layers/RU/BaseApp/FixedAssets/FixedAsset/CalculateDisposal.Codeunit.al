...
codeunit 5605 "Calculate Disposal"
{
        ...
        if FALedgEntry.Find('-') then begin
            repeat
                ...
                if FALedgEntry."Disposal Entry No." > MaxDisposalNo then begin
                    ...
                    if FALedgEntry."Document Type" in [FALedgEntry."Document Type"::Invoice, FALedgEntry."Document Type"::"Credit Memo"] then begin
                        DisposalAmount := DisposalAmount + FALedgEntry.Amount;
                        SalesDisposal := true;
                    end;
                end;
    ...
    begin
        ...
        if DeprBook."G/L Integration - Disposal" and DeprBook."G/L Integration - Acq. Cost" then
            GainLoss := FADeprBook."Book Value";
        EntryAmounts[2] := GainLoss;
    ...
    begin
        ...
        for i := 2 to 14 do begin
            ...
            if FALedgEntry.Find('-') then
                repeat
                    if FALedgEntry."Transaction No." = TransactionNo then begin
                        EntryNumbers[i] := FALedgEntry."Entry No.";
                        EntryAmounts[i] := -FALedgEntry.Amount;
                    end;
                until FALedgEntry.Next() = 0;
