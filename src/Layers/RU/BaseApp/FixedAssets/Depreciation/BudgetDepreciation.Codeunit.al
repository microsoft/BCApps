            ...
            FADeprBook."Depreciation Method"::"DB2/SL":
                ...
                if EndingDate1 > 0D then
                    CalculateNormalDepr.Calculate(
                      DeprAmount1, NumberOfDays, FANo, DeprBookCode, EndingDate1, DummyEntryAmounts, 0D, 0, CalcDate('<-CM>', EndingDate1));
                if EndingDate2 > 0D then
                    CalculateNormalDepr.Calculate(
                      DeprAmount2, NumberOfDays, FANo, DeprBookCode, EndingDate2, DummyEntryAmounts, 0D, 0, CalcDate('<-CM>', EndingDate2));
            end;
        end;
    end;
