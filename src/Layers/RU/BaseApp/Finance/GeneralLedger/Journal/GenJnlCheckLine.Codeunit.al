...
#pragma warning restore AA0074
    ...
    begin
        ...
        if GenJnlLine."Account Type" = GenJnlLine."Account Type"::Customer then
            CheckCustAgrmt(GenJnlLine, GenJnlLine."Account No.");

        if GenJnlLine."Bal. Account Type" = GenJnlLine."Bal. Account Type"::Customer then
            CheckCustAgrmt(GenJnlLine, GenJnlLine."Bal. Account No.");

        if GenJnlLine."Account Type" = GenJnlLine."Account Type"::Vendor then
            CheckVendAgrmt(GenJnlLine, GenJnlLine."Account No.");

        if GenJnlLine."Bal. Account Type" = GenJnlLine."Bal. Account Type"::Vendor then
            CheckVendAgrmt(GenJnlLine, GenJnlLine."Bal. Account No.");
        ...
        if GenJnlLine."Additional VAT Ledger Sheet" then
            GenJnlLine.TestField("Corrected Document Date", ErrorInfo.Create());

        if GenJnlLine."Additional VAT Ledger Sheet" and GenJnlLine."Include In Other VAT Ledger" then
            Error(ErrorInfo.Create(Text12400, true, GenJnlLine));

        if GenJnlLine."Export Status" in [GenJnlLine."Export Status"::New, GenJnlLine."Export Status"::Exported] then
            GenJnlLine.FieldError("Export Status", ErrorInfo.Create());
            ...
            GenJnlLine."Account Type"::Customer, GenJnlLine."Account Type"::Vendor, GenJnlLine."Account Type"::Employee:
                begin
                    ...
                    if GLSetup."Enable Russian Accounting" and
                       (GenJnlLine."Document Type" = GenJnlLine."Document Type"::Payment) and GenJnlLine.Prepayment
                    then begin
                        GenJnlLine.TestField("Applies-to Doc. Type", 0, ErrorInfo.Create());
    ...
    begin
        ...
        case Cust."Agreement Posting" of
            Cust."Agreement Posting"::Mandatory:
                begin
                    GenJnlLine2.TestField("Agreement No.", ErrorInfo.Create());
