...
table 179 "Reversal Entry"
{
    ...
    var
        ...
        GlobalTaxDiffLedgerEntry: Record "Tax Diff. Ledger Entry";
        GlobalGLItemLedgerRelation: Record "G/L - Item Ledger Relation";
...
#pragma warning restore AA0074
    ...
    begin
        ...
        GlobalVATEntry.SetRange("VAT Allocation Type", GlobalVATEntry."VAT Allocation Type"::Charge);
        if not GlobalVATEntry.IsEmpty() then
            Error('');
    ...
    begin
        ...
        if VATAllocOnCost then begin
            SourceCodeSetup.Get();
            Completed := false;
            TempGLRegister.DeleteAll();
            GlobalGLRegister.Get(Number);
            GLRegister2.Get(Number);
            repeat
                if GLRegister2.Next(-1) <> 0 then
                    if (GLRegister2."Source Code" = SourceCodeSetup."Inventory Post Cost") and
                       (GLRegister2.SystemCreatedAt = GlobalGLRegister.SystemCreatedAt) and
                       (GLRegister2."User ID" = GlobalGLRegister."User ID")
                    then begin
                        TempGLRegister := GLRegister2;
                        TempGLRegister.Insert();
                    end else
                        Completed := true;
            until Completed;
        end;
        ...
        InsertFromTaxDiffEntry(TempTransactionNoInteger, Number, RevType, NextLineNo);
    ...
    begin
        ...
        if IsVATAllocOnCost then
            if GlobalValueEntry.Find('-') then begin
                SkipCheck := false;
                OnBeforeCheckEntries(Rec, DATABASE::"VAT Entry", SkipCheck);
                if not SkipCheck then
                    repeat
                        CheckItem(GlobalValueEntry);
                    until GlobalValueEntry.Next() = 0;
            end;

        if GlobalTaxDiffLedgerEntry.Find('-') then begin
            SkipCheck := false;
            OnBeforeCheckEntries(Rec, DATABASE::"Tax Diff. Ledger Entry", SkipCheck);
            if not SkipCheck then
                repeat
                    CheckTaxDiff(GlobalTaxDiffLedgerEntry);
                until GlobalTaxDiffLedgerEntry.Next() = 0;
        end;
    ...
    begin
        ...
        if (GlobalGLEntry."Journal Batch Name" <> '') or IsVATAllocOnCost then
            exit;
    ...
    begin
        ...
        FALedgerEntry.CheckRealizedVAT(FALedgerEntry."Entry No.");
    ...
    begin
        ...
        GlobalVATEntry.SetRange("Entry No.", GLRegister."From VAT Entry No.", GLRegister."To VAT Entry No.");
        GlobalVATEntry.SetRange("VAT Allocation Type", GlobalVATEntry."VAT Allocation Type"::Charge);
        VATAllocOnCost := not GlobalVATEntry.IsEmpty();
        GlobalVATEntry.Reset();
        if (GLRegister."Journal Batch Name" = '') and not VATAllocOnCost then
    ...
    begin
        ...
        if RevType = RevType::Transaction then begin
            ...
            GlobalTaxDiffLedgerEntry.SetRange("Transaction No.", Number);
            GlobalValueEntry.SetRange("Entry No.", 0);
        end else begin
            GlobalGLRegister.Get(Number);
            ...
            GlobalTaxDiffLedgerEntry.SetCurrentKey("Transaction No.");
            if not GlobalGLEntry.Find('-') then
                FromTransactionNo := GlobalGLEntry."Transaction No." + 1
            else begin
                FromTransactionNo := GlobalGLEntry."Transaction No.";
                GlobalGLEntry.Find('+');
            end;
            GlobalTaxDiffLedgerEntry.SetRange("Transaction No.", FromTransactionNo, GlobalGLEntry."Transaction No.");
        end;
    ...
    procedure CopyReverseFilters2(var ValueEntry: Record "Value Entry"; var TaxDiffLedgerEntry: Record "Tax Diff. Ledger Entry")
    begin
    ...
    begin
        ...
        case "Entry Type" of
            ...
            "Entry Type"::"Tax Difference":
                begin
                    if GlobalTaxDiffLedgerEntry.Get("Entry No.") then;
                    if TaxDifference.Get(GlobalTaxDiffLedgerEntry."Tax Diff. Code") then;
                    exit(StrSubstNo(CaptionTxt, TaxDifference.TableCaption(), TaxDifference.Code, TaxDifference.Description));
                end;
            else begin
    ...
    local procedure CheckTaxDiff(TaxDiffLedgerEntry: Record "Tax Diff. Ledger Entry")
    var
        TaxDifference: Record "Tax Difference";
        TaxDiffLedgerEntry2: Record "Tax Diff. Ledger Entry";
    begin
        TaxDifference.Get(TaxDiffLedgerEntry."Tax Diff. Code");
        CheckPostingDate(TaxDiffLedgerEntry."Posting Date", TableCaption(), TaxDiffLedgerEntry."Entry No.");
        if TaxDiffLedgerEntry.Reversed then
            AlreadyReversedEntry(TableCaption, TaxDiffLedgerEntry."Entry No.");
        TaxDiffLedgerEntry2.SetCurrentKey("Tax Diff. Code", "Source Type", "Source No.");
        TaxDiffLedgerEntry2.SetRange("Tax Diff. Code", TaxDiffLedgerEntry."Tax Diff. Code");
        TaxDiffLedgerEntry2.SetRange("Source Type", TaxDiffLedgerEntry."Source Type");
        TaxDiffLedgerEntry2.SetRange("Source No.", TaxDiffLedgerEntry."Source No.");
        TaxDiffLedgerEntry2.SetFilter("Entry No.", '>%1', TaxDiffLedgerEntry."Entry No.");
        TaxDiffLedgerEntry2.SetRange(Reversed, false);
        if TaxDiffLedgerEntry2.FindLast() then
            Error(CannotReverseTaxDiffEntryErr, TaxDiffLedgerEntry."Entry No.", TaxDiffLedgerEntry2."Entry No.");
    end;

    [Scope('OnPrem')]
    procedure ShowTaxDiffEntries()
    begin
        PAGE.Run(0, GlobalTaxDiffLedgerEntry);
    end;

    [Scope('OnPrem')]
    procedure SetVATSettlReverseFilters(VATEntryNo: Integer; FirstGLEntryNo: Integer; LastGLEntryNo: Integer)
    begin
        GlobalVATEntry.SetRange("Entry No.", VATEntryNo);
        GlobalGLEntry.SetRange("Entry No.", FirstGLEntryNo, LastGLEntryNo);
        GlobalCustLedgerEntry.SetRange("Entry No.", -1);
        GlobalVendorLedgerEntry.SetRange("Entry No.", -1);
        GlobalBankAccountLedgerEntry.SetRange("Entry No.", -1);
        GlobalFALedgerEntry.SetCurrentKey("G/L Entry No.");
        GlobalFALedgerEntry.SetRange("G/L Entry No.", -1);
        GlobalMaintenanceLedgerEntry.SetCurrentKey("G/L Entry No.");
        GlobalMaintenanceLedgerEntry.SetRange("G/L Entry No.", -1);
        GlobalTaxDiffLedgerEntry.SetCurrentKey("Transaction No.");
        GlobalTaxDiffLedgerEntry.SetRange("Transaction No.", -1);
    end;
    ...
    local procedure InsertFromTaxDiffEntry(var TempTransactionNoInteger: Record "Integer" temporary; Number: Integer; RevType: Option Transaction,Register; var NextLineNo: Integer)
    var
        TaxDifference: Record "Tax Difference";
    begin
        TempTransactionNoInteger.FindSet();
        repeat
            if RevType = RevType::Transaction then
                GlobalTaxDiffLedgerEntry.SetRange("Transaction No.", TempTransactionNoInteger.Number);
            if GlobalTaxDiffLedgerEntry.FindSet() then
                repeat
                    Clear(TempReversalEntry);
                    if RevType = RevType::Register then
                        TempReversalEntry."G/L Register No." := Number;
                    ...
                    if not TaxDifference.Get(GlobalTaxDiffLedgerEntry."Tax Diff. Code") then
                        Error(CannotReverseDeletedErr, GlobalTaxDiffLedgerEntry.TableCaption(), TaxDifference.TableCaption());
                    TempReversalEntry."Account No." := TaxDifference.Code;
                    TempReversalEntry."Account Name" := CopyStr(TaxDifference.Description, 1, MaxStrLen(TempReversalEntry."Account Name"));
                    TempReversalEntry.CopyFromTaxDiffEntry(GlobalTaxDiffLedgerEntry);
                    TempReversalEntry."Line No." := NextLineNo;
                    NextLineNo := NextLineNo + 1;
                    TempReversalEntry.Insert();
                until GlobalTaxDiffLedgerEntry.Next(1) = 0;
        until TempTransactionNoInteger.Next() = 0;
    end;
    ...
    begin
        FromValueEntryNo := 0;
        if TempGLRegister.FindSet() then
            repeat
                GlobalGLEntry.SetRange("Entry No.", TempGLRegister."From Entry No.", TempGLRegister."To Entry No.");
                if GlobalGLEntry.FindSet() then
                    repeat
                        Clear(TempReversalEntry);
                        if RevType = RevType::Register then
                            TempReversalEntry."G/L Register No." := TempGLRegister."No.";
                        TempReversalEntry."Reversal Type" := RevType;
                        TempReversalEntry."Entry Type" := TempReversalEntry."Entry Type"::"G/L Account";
                        if not GLAccount.Get(GlobalGLEntry."G/L Account No.") then
                            Error(CannotReverseDeletedErr, GlobalGLEntry.TableCaption(), GLAccount.TableCaption());
                        ...
                        if GlobalGLEntry.Amount > 0 then begin
                            GlobalGLItemLedgerRelation.Reset();
                            GlobalGLItemLedgerRelation.SetRange("G/L Entry No.", GlobalGLEntry."Entry No.");
                            if GlobalGLItemLedgerRelation.FindFirst() then begin
                                Clear(TempReversalEntry);
                                GlobalValueEntry.Get(GlobalGLItemLedgerRelation."Value Entry No.");
                                if FromValueEntryNo = 0 then
                                    FromValueEntryNo := GlobalValueEntry."Entry No.";
                                ToValueEntryNo := GlobalValueEntry."Entry No.";
                                if RevType = RevType::Register then
                                    TempReversalEntry."G/L Register No." := TempGLRegister."No.";
                                TempReversalEntry."Reversal Type" := RevType;
                                TempReversalEntry."Entry Type" := TempReversalEntry."Entry Type"::"Value Entry";
                                Item.Get(GlobalValueEntry."Item No.");
                                TempReversalEntry."Account No." := Item."No.";
                                TempReversalEntry."Account Name" := Item.Description;
                                TempReversalEntry.CopyFromValueEntry(GlobalValueEntry);
                                TempReversalEntry."Line No." := NextLineNo;
                                NextLineNo := NextLineNo + 1;
                                TempReversalEntry.Insert();
                            end;
    ...
    begin
        ...
        "Transaction No." := GlobalGLEntry."Transaction No.";
    ...
    [Scope('OnPrem')]
    procedure CopyFromTaxDiffEntry(TaxDiffLedgerEntry: Record "Tax Diff. Ledger Entry")
    begin
        "Entry No." := TaxDiffLedgerEntry."Entry No.";
        "Posting Date" := TaxDiffLedgerEntry."Posting Date";
        "Source Code" := TaxDiffLedgerEntry."Source Code";
        "Journal Batch Name" := TaxDiffLedgerEntry."Journal Batch Name";
        "Transaction No." := TaxDiffLedgerEntry."Transaction No.";
        case TaxDiffLedgerEntry."Source Type" of
            TaxDiffLedgerEntry."Source Type"::"Future Expense":
                "Source Type" := "Source Type"::"Future Expense";
        end;
        "Source No." := TaxDiffLedgerEntry."Source No.";
        Description := TaxDiffLedgerEntry.Description;
        "Amount (LCY)" := TaxDiffLedgerEntry."Tax Amount";
        "Debit Amount (LCY)" := TaxDiffLedgerEntry."Asset Tax Amount";
        "Credit Amount (LCY)" := TaxDiffLedgerEntry."Liability Tax Amount";
        "VAT Amount" := TaxDiffLedgerEntry."Disposal Tax Amount";
        "Document No." := TaxDiffLedgerEntry."Document No.";
    end;
