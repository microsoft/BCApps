...
page 489 "Column Layout"
{
    ...
    begin
        ...
        if CurrentColumnName <> '' then
            if ColumnLayoutName.Get(CurrentColumnName) and (ColumnLayoutName.Description <> '') then
                CurrentPageCaption := StrSubstNo('%1 (%2)', ColumnLayoutName.Description, ColumnLayoutName.Name);
        if CurrentPageCaption = '' then
            CurrentPageCaption := CurrentColumnName;
        if CurrentPageCaption <> '' then
