...
report 596 "Exch. Rate Adjustment"
{
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(DimForPositive; GainsDimensions)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Dimensions for Gains';
                        ToolTip = 'Specifies dimensions for posting exchange rate adjustment gains.';

                        trigger OnAssistEdit()
                        var
                            DimSelectionBuf: Record "Dimension Selection Buffer";
                        begin
                            DimSelectionBuf.SetDimSelectionChange(3, REPORT::"Exch. Rate Adjustment", GainsDimensions);
                        end;
                    }
                    field(DimForNegative; LossesDimensions)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Dimensions for Losses';
                        ToolTip = 'Specifies dimensions for posting exchange rate adjustment losses.';

                        trigger OnAssistEdit()
                        var
                            DimSelectionBuf: Record "Dimension Selection Buffer";
                        begin
                            DimSelectionBuf.SetDimSelectionChange(3, -REPORT::"Exch. Rate Adjustment", LossesDimensions);
                        end;
                    }
                }
            }
        ...
        begin
            ...
            GainsDimensions := DimSelectionBuf.GetDimSelectionText(3, REPORT::"Exch. Rate Adjustment", '');
            LossesDimensions := DimSelectionBuf.GetDimSelectionText(3, -REPORT::"Exch. Rate Adjustment", '')
        end;
    }
    ...
    var
        ...
        GainsDimensions: Text[250];
        LossesDimensions: Text[250];
        PostingDate: Date;
    ...
    begin
        ...
        ExchRateAdjmtParameters2."Gains Dimensions" := GainsDimensions;
        ExchRateAdjmtParameters2."Losses Dimensions" := LossesDimensions;
        if GeneralLedgerSetup."Journal Templ. Name Mandatory" then begin
