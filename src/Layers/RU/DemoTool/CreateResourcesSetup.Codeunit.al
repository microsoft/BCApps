    ...
    begin
        ...
        if ResourcesSetup."Resource Nos." = '' then
            if not NoSeries.Get(XRES) then
                CreateNoSeries.InitBaseSeries(ResourcesSetup."Resource Nos.", XRJ + '-02', XResource, XR10, XR9990, '', '', 10, Enum::"No. Series Implementation"::Sequence)
            else
                ResourcesSetup."Resource Nos." := XRJ + '-02';
