codeunit 101901 "Create Sequence of Actions"
{
    ...
    begin
        ...
        "Interface Russian Accounting"."Before Posting"();
        "Interface Tax Accounting"."Before Posting"();
        ...
        for ActualDate := CA.AdjustDate(19011201D) to CA.AdjustDate(19030126D) do begin
            ...
            Window.Update(
              2, Round(
                ...
                (CA.AdjustDate(19030126D) - CA.AdjustDate(19011201D)) * 10000, 1));

            "Interface Basis Data".Post(ActualDate);
            if DemoDataSetup."Russian Accounting" then
                "Interface Russian Accounting".Post(ActualDate);
            if DemoDataSetup."Tax Accounting" then
                "Interface Tax Accounting".Post(ActualDate);
        ...
        if DemoDataSetup."Russian Accounting" then
            "Interface Russian Accounting"."After Posting"();
        if DemoDataSetup."Tax Accounting" then
            "Interface Tax Accounting"."After Posting"();
        if DemoDataSetup.Manufacturing then
