codeunit 101550 "Create Contact"
{
    ...
    begin
        ...
        if (Type = Contact.Type::Person.AsInteger()) and ("Company No." in ['CT000001', 'CT000049', 'CT000053']) then
            case ContBusRel."Link to Table" of
                ContBusRel."Link to Table"::Customer:
                    begin
                        ...
                        Contact.County := Customer.County;
                        Contact."Country/Region Code" := '';
                        if AddressID.Get(DATABASE::Customer, Customer.GetPosition()) then begin
                            AddressID."Table No." := DATABASE::Contact;
                            AddressID."Table Key" := Contact.GetPosition();
                            AddressID.Insert();
                        end;
                    end;
                ContBusRel."Link to Table"::Vendor:
                    begin
                        ...
                        Contact."Post Code" := Vendor."Post Code";
                        Contact.City := Vendor.City;
                        Contact.County := Vendor.County;
                        Contact."Country/Region Code" := '';
                        if AddressID.Get(DATABASE::Vendor, Vendor.GetPosition()) then begin
                            AddressID."Table No." := DATABASE::Contact;
                            AddressID."Table Key" := Contact.GetPosition();
                            AddressID.Insert();
                        end;
