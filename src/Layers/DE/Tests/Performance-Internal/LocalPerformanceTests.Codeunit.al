codeunit 139094 "Local Performance Tests"
{
    ...
    [Test]
    [HandlerFunctions('VendorPaymentsListHandler')]
    [TransactionModel(TransactionModel::AutoRollback)]
    [Scope('OnPrem')]
    procedure TestVendorPaymentsList()
    var
    ...
    begin
        ...
        PopulateVendorLedgerEntry(VendorLedgerEntry);

        LibraryPerformanceProfiler.StartProfiler(true);
        Report.RunModal(Report::"Vendor Payments List", true, false, VendorLedgerEntry);
        TraceDumpFilePath := LibraryPerformanceProfiler.StopProfiler(
            PerfProfilerEventsTest, 'TestVendorPaymentsList',
            PerfProfilerEventsTest."Object Type"::Report, REPORT::"Vendor Payments List", true);
    end;
    ...
    local procedure PopulateVendorLedgerEntry(var VendorLedgerEntry: Record "Vendor Ledger Entry")
    var
        VendorCode: Code[20];
    begin
        VendorLedgerEntry.DeleteAll();

        VendorCode := CreateVendor();

        CreateSingleVendorLedgerEntry(VendorLedgerEntry, VendorCode, VendorLedgerEntry."Document Type"::Payment);
        CreateSingleVendorLedgerEntry(VendorLedgerEntry, VendorCode, VendorLedgerEntry."Document Type"::Payment);
        CreateSingleVendorLedgerEntry(VendorLedgerEntry, VendorCode, VendorLedgerEntry."Document Type"::Invoice);
        CreateSingleVendorLedgerEntry(VendorLedgerEntry, VendorCode, VendorLedgerEntry."Document Type"::Invoice);
    end;
    ...
    local procedure CreateSingleVendorLedgerEntry(var VendorLedgerEntry: Record "Vendor Ledger Entry"; VendorCode: Code[20]; DocumentType: Enum "Gen. Journal Document Type")
    begin
        VendorLedgerEntry.Init();
        VendorLedgerEntry."Entry No." := System.Random(10000);
        VendorLedgerEntry."Vendor No." := VendorCode;
        VendorLedgerEntry."Posting Date" := WorkDate();
        VendorLedgerEntry."Document Type" := DocumentType;
        VendorLedgerEntry."Applies-to Doc. No." := LibraryUTUtility.GetNewCode();
