...
codeunit 286 StatelessNoSeriesManagement
{
...
#pragma warning restore AL0432
    ...
    procedure UpdateLine(var NoSeries: Record "No. Series"; var StartDate: Date; var StartNo: Code[20]; var EndNo: Code[20]; var LastNoUsed: Code[20]; var WarningNo: Code[20]; var IncrementByNo: Integer; var LastDateUsed: Date)
    var
        AllowGaps: Boolean;
    begin
        UpdateLine(NoSeries, StartDate, StartNo, EndNo, LastNoUsed, WarningNo, IncrementByNo, LastDateUsed, AllowGaps);
    end;

    procedure UpdateLine(var NoSeries: Record "No. Series"; var StartDate: Date; var StartNo: Code[20]; var EndNo: Code[20]; var LastNoUsed: Code[20]; var WarningNo: Code[20]; var IncrementByNo: Integer; var LastDateUsed: Date; var AllowGaps: Boolean)
    var
        NoSeriesLine: Record "No. Series Line";
    begin
        case NoSeries."No. Series Type" of
...
#pragma warning restore AL0432
    ...
    begin
        ...
        if not SequenceNoSeriesManagement.TryGetCurrentSequenceNo(NoSeriesLine, LastSeqNoUsed) then begin
            if not NumberSequence.Exists(NoSeriesLine."Sequence Name") then
                SequenceNoSeriesManagement.CreateNewSequence(NoSeriesLine);
            SequenceNoSeriesManagement.TryGetCurrentSequenceNo(NoSeriesLine, LastSeqNoUsed);
        end;
        if LastSeqNoUsed >= NoSeriesLine."Starting Sequence No." then
            exit(SequenceNoSeriesManagement.GetFormattedNo(NoSeriesLine, LastSeqNoUsed));
        exit('');
    end;
