...
codeunit 285 StatefulNoSeriesManagement implements "No. Series - Single"
{
    ...
    begin
        NoSeries.Get(TryNoSeriesCode);
        TryNo := GetNextNo(TryNoSeriesCode, TrySeriesDate, false, NoSeries);
    end;
    ...
    begin
        IsHandled := false;
        OnBeforeInitSeries(DefaultNoSeriesCode, OldNoSeriesCode, NewDate, NewNo, NewNoSeriesCode, NoSeries, IsHandled, GlobalNoSeriesCode);
        if IsHandled then
            exit;
        ...
        if NewNo = '' then begin
            ...
            if OldNoSeriesCode <> '' then begin
                ...
                FilterSeries(NoSeries);
                NoSeries.Code := OldNoSeriesCode;
                if not NoSeries.FindFirst() then
                    NoSeries.Get(DefaultNoSeriesCode);
            end;
            NewNo := GetNextNo(NoSeries.Code, NewDate, true, NoSeries);
...
#pragma warning restore AA0175
    ...
    begin
        ...
        NoSeries.FindFirst();
        NewNo := GetNextNo(NoSeries.Code, 0D, true, NoSeries);
    end;

    procedure FilterSeries()
    begin
        FilterSeries(GlobalNoSeries);
    end;

    local procedure FilterSeries(var NoSeries: Record "No. Series")
    var
        NoSeriesRelationship: Record "No. Series Relationship";
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeFilterSeries(NoSeries, GlobalNoSeriesCode, IsHandled);
        if IsHandled then
            exit;
        ...
        if NoSeries.Get(GlobalNoSeriesCode) then
            NoSeries.Mark := true;
    ...
    begin
        exit(GetNextNo(NoSeriesCode, SeriesDate, ModifySeries, GlobalNoSeries));
    end;

    procedure GetNextNo(NoSeriesCode: Code[20]; SeriesDate: Date; ModifySeries: Boolean; var NoSeries: Record "No. Series") Result: Code[20]
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        ...
        exit(DoGetNextNo(NoSeriesCode, SeriesDate, ModifySeries, false, NoSeries));
    end;
...
#pragma warning restore AL0432
    ...
    begin
        exit(DoGetNextNo(NoSeriesCode, SeriesDate, ModifySeries, NoErrorsOrWarnings, GlobalNoSeries));
    end;

    procedure DoGetNextNo(NoSeriesCode: Code[20]; SeriesDate: Date; ModifySeries: Boolean; NoErrorsOrWarnings: Boolean; var NoSeries: Record "No. Series"): Code[20]
    var
    ...
    begin
        SaveNoSeries(GlobalNoSeries);
    end;

    procedure SaveNoSeries(var NoSeries: Record "No. Series")
    var
...
#pragma warning restore AL0432
    ...
    // TODO: Obsolete this function to local apac codeunit
    [Scope('OnPrem')]
    procedure ReverseGetNextNo(NoSeriesCode: Code[20]; SeriesDate: Date; ModifySeries: Boolean): Code[20] // APAC
    var
        NoSeries: Record "No. Series";
        NoSeriesLine: Record "No. Series Line";
    begin
...
#pragma warning restore AL0432
    ...
    begin
        ...
        exit(GetNextNo(NoSeriesCode, WorkDate(), false, GlobalNoSeries));
    end;
